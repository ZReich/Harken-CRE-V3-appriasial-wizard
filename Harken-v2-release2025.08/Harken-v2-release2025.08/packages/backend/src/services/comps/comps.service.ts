...
import { clusterDetailsSchema, combinedSchema, compsListSchema, downloadCompSchema, mapBoundsSchema, validateCompSchema } from './comps.validations';
...
export default class CompsService {
	...
	public extractPdfPropertiesData = async (
		...
		try {
			...
			**IMPORTANT:**
			...
			13. For "building_sub_type" and "building_sub_type_custom":
				- Treat the building subtype as the more specific descriptor of the property’s use (more specific than the main building type).
				- Look for it in any “Sub-Type” / “Building Subtype” style field, or as the text that follows the building type when they appear together in the same segment (for example separated by a hyphen or slash).
				- When building type and subtype appear together, extract only the subtype portion (the part after the building type) and, if it is split across multiple lines, join the relevant lines into one continuous phrase.
				- Stop the subtype before any size or area information (such as values followed by “SF”, “Sq Ft”, “Acre”, or similar).
				- Copy the subtype exactly as printed into "building_sub_type_custom", preserving all characters, and set "building_sub_type" to null.
				- If no clear subtype exists, set both fields to null and do not guess or output placeholder text.
			...
			32. If "Transaction date" or "Date sold" is available, add it to the "date_sold" field.
			...
			8. For the "building_type" field, if the value is "multi-family" or "multi family" set it to "multi_family".
	...
	public saveExtractedPdfComp = async (
		...
		let data: IError | ICompSuccess<IExtractedCompSuccessData>;
		try {
			const compData = request.body;
			...
			const comps = await this.storage.saveExtractedComp(compAttributes);
			const { compId } = comps;
			if (!compId) {
				data = {
					statusCode: StatusCodeEnum.BAD_REQUEST,
			...
			data = {
				statusCode: StatusCodeEnum.OK,
				message: comps.message || CompEnum.COMP_SAVE_SUCCESS,
				data: comps,
			};
	...
	public validateComp = async (
		...
		try {
			...
			const comp = await this.storage.validateComp(compAttributes);
