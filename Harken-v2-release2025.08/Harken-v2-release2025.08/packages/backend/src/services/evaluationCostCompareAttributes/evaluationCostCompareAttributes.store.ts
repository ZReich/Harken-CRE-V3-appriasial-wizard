...
import { ICostComparisonAttributes } from './IEvaluationCostCompareAttributes';
const CostComparisonAttributes = database.eval_cost_approach_comparison_attributes;

export default class EvaluationCostComparativeStore {
	...
	public async update(
		...
		try {
			const { comparison_key, evaluation_cost_approach_id, ...rest } = attributes;
			const whereClause = { comparison_key, evaluation_cost_approach_id };
