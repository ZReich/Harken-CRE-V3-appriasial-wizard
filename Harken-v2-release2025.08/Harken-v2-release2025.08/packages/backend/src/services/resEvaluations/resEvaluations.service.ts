...
export default class ResEvaluationsService {
	...
	public updateExhibit = async (
		...
		try {
			...
			const updateData = await this.evaluationFilesStorage.updateEvaluationFiles(attributes);
			if (!updateData) {
				data = {
					statusCode: StatusCodeEnum.BAD_REQUEST,
	...
	public previewReport = async (
		...
		try {
			// Check if user is authenticated (for public route compatibility)
			const userRole = request.user?.role;
			
			// Role validations - only apply if user is authenticated
			if (userRole && userRole === RoleEnum.DATA_ENTRY) {
				data = {
					statusCode: StatusCodeEnum.UNAUTHORIZED,
					message: ErrorMessageEnum.PERMISSION_DENIED,
				};
