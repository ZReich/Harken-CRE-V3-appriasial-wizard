...
const EvaluationRentRoll = database.evaluation_rent_roll;
import HelperFunction from '../../utils/common/helper';
import IRentRolls from './IRentRolls';
import { Op } from 'sequelize';
import { ISaveRentRoll } from '../evaluations/IEvaluationsService';
import { RentRollEnums } from '../../utils/enums/CommonEnum';

const helperFunction = new HelperFunction();
export default class EvaluationRentRollsStore {
	...
	public async create(attributes: Partial<ISaveRentRoll>): Promise<IRentRolls> {
		try {
			return await EvaluationRentRoll.create(attributes);
		} catch (e) {
			//logging error
			helperFunction.log({
				message: e.message,
	...
	/**
	 * @description find evaluation rent roll
	 * @param attributes
	 * @returns
	 */
	public async find(attributes: Partial<IRentRolls>): Promise<IRentRolls> {
		try {
			return await EvaluationRentRoll.findOne({
				where: attributes,
			});
		} catch (e) {
			//logging error
	...
	public async findAll(attributes: Partial<IRentRolls>): Promise<IRentRolls[]> {
		try {
			return await EvaluationRentRoll.findAll({
				where: attributes,
			});
		} catch (e) {
			//logging error
	...
	public async update(attributes: Partial<IRentRolls>): Promise<boolean> {
		try {
			...
			return await EvaluationRentRoll.update(rest, {
				where: {
					id: id,
				},
			});
	...
	public async remove(
		...
		objectId: number,
		objectColumn = RentRollEnums.EVALUATION_RENT_ROLL_TYPE_ID,
	): Promise<boolean> {
		try {
			const instances = await EvaluationRentRoll.findAll({
				attributes: ['id'],
				where: {
