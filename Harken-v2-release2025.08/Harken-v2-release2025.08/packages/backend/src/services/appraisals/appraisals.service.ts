...
export default class AppraisalsService {
	...
	public updateExhibit = async (
		...
		try {
			// Validate the request schema
			const params = await helperFunction.validate(updateExhibitSchema, request.body);
			if (!params.value) {
				data = {
					statusCode: StatusCodeEnum.BAD_REQUEST,
					message: ErrorMessageEnum.INVALID_REQUEST,
			...
			const { fileId, fileName }: IUpdateExhibits = request.body;
	...
	public updateApproaches = async (
		...
		try {
			...
			for (const approach of appraisalData.appraisal_approaches) {
				...
				if (approach.type === AppraisalsEnum.INCOME) {
					if (approach?.appraisal_income_approach) {
						...
						const incomeAttributes = {
							appraisal_approach_id: approach.id,
						}
						const saveIncomeData = await this.appraisalIncomeStore.createIncomeApproach(incomeAttributes);
						if (
							data.comp_type == AppraisalsEnum.LAND_ONLY
							...
							const attributes = {
								appraisal_income_approach_id: saveIncomeData.id,
								type: data.land_type,
								sf_source: data.land_size,
								comments: AppraisalsEnum.LAND_ONLY_TYPE as string,
								monthly_income: 0,
							...
							// Prepare attributes for calculating the income approach
							const incomeAttributes = { data, income: saveIncomeData };
							// Calculate the incremental value using the income approach
							this.calculateIncomeApproach(incomeAttributes);
						}
					}
	...
	public calculateIncomeApproach = async (incomeAttributes: ICalculateIncome) => {
		try {
			...
			const adjustedGrossAmount = annualIncomeTotal + (incomeData?.other_total_annual_income || 0) - vacancyAmount;
	...
	public updateExhibitsPositions = async (
		...
		try {
			...
			data = {
				statusCode: StatusCodeEnum.OK,
				message: AppraisalEnum.EXHIBIT_POSITION_UPDATE_SUCCESS,
			};
			//logging information
			helperFunction.log({
				message: data.message,
		...
		try {
			...
			for (const item of operatingExpenses) {
				...
				expenseRows += `<tr><td>${name || AppraisalsEnum.NA}</td>
					...
					${compType === AppraisalsEnum.BUILDING_WITH_LAND ? `${compareBasis}` : ''}
					<td>${totalPerSqFt}</td>
					<td>${comments}</td></tr>`;
			}
			...
			if (
				...
				comparisonIndicateValue += `<tr><td><b>Indicated Value Per Unit</b></td>
				...
				comparisonIndicateValue += `<tr><td><b>Indicated Value Per Bed</b></td>
			...
			htmlContent +=  otherIncomeSource ? `<h4>OTHER INCOME</h4>` : '';
			htmlContent += `<table style="width:100%; border-collapse: collapse;" border="1">`;
			htmlContent += otherIncomeSource ? `<tr><td><b>Type</b></td>
					<td><b>Monthly Income</b></td>
					<td><b>Annual Income</b></td>
					<td>${totalIncomeLabel}</td>
					<td><b>Comments</b></td></tr>
			...
			htmlContent += otherIncomeSource ? `<tr><td colspan="6"><b>Total Other Income: </b> ${otherTotalAnnualIncome}</td></tr>` : '';
			htmlContent += `<tr><td colspan="6"><b>Effective Income: </b> ${adjustedGrossAmount}</td></tr>
			</table><br>
			...
			<tr><td><b>Indicated Value ${indicatedValueLabel}</b></td>
