...
const EvaluationSalesApproach = database.res_evaluation_sales_approaches;
const SalesApproachComps = database.res_eval_sales_approach_comps;
const SalesApproachSubjectAdj = database.res_eval_sales_approach_subject_adj;
const SalesApproachCompAdj = database.res_eval_sales_approach_comp_adj;
const Comps = database.res_comps;
const Zoning = database.res_zoning;
const ResCompAmenities = database.res_comp_amenities;
const SubjectQualitativeAdj = database.res_eval_sales_approach_qualitative_sub_adj;
const SaleQualitativeCompsAdj = database.res_eval_sales_approach_qualitative_comp_adj;
const SalesComparisonAttribute = database.res_eval_sales_approach_comparison_attributes;
const ResExtraCompAmenities = database.res_eval_sales_approach_comp_amenities;
import HelperFunction from '../../utils/common/helper';
import {
	IEvalSalesApproach,
	ISalesApproachesRequest,
	ISalesApproachesWeightRequest,
} from './IResEvaluationSalesService';
import { EvaluationsEnum } from '../../utils/enums/EvaluationsEnum';

const helperFunction = new HelperFunction();
export default class ResEvaluationSalesStore {
	...
	public async createSalesApproach(
		...
		try {
			...
			const nestedComps =
				comp_data?.map((comp) => {
					const { extra_amenities = [] } = comp;

					const updatedAmenities = extra_amenities
						.filter((item) => item?.another_amenity_name?.trim()) // Remove empty/null/whitespace-only entries
						.map((item, index) => ({
							...item,
					...
					return {
						...comp,
						extra_amenities: updatedAmenities,
					...
					include: [
						...
						{ model: SubjectQualitativeAdj, as: 'subject_qualitative_adjustments' },
						{
							...
							include: [
								...
								{ model: ResExtraCompAmenities, as: 'extra_amenities' },
							],
						},
					],
				},
	...
	public async findByAttribute(
		...
		try {
			...
			const linkedComps = await SalesApproachComps.findAll({
				where: { res_evaluation_sales_approach_id: evaluationSalesApproach.id },
				attributes: [
					'id',
					'comp_id',
					'adjustment_note',
			...
			const compDetails = await Promise.all(
				linkedComps.map(async (comp) => {
					const compData = await Comps.findOne({
						...
						include: [Zoning, ResCompAmenities],
					});
