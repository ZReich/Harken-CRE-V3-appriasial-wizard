...
import { ICostCompsAdjustment } from './IResEvaluationCostApproachCompAdj';

const EvaluationCostCompAdj = database.res_eval_cost_approach_comp_adj;
export default class ResEvaluationCostCompAdjStorage {
	...
	public async updateAdjustments(
		...
		try {
			const { adj_key, res_eval_cost_approach_comp_id, ...rest } = attributes;
			return await EvaluationCostCompAdj.update(rest, {
				where: {
					adj_key,
					res_eval_cost_approach_comp_id,
				},
			});
		} catch (e) {
			return e.message || e;
