...
import { DeleteAppraisalEnum, DeleteCompEnum, DeleteTemaplateEnum, DeleteImageEnum } from '@/pages/comps/enum/CompsEnum';

interface DeleteModalContentProps {
  close: () => void;
  deleteComps: () => void;
  label: string;
  type?: 'appraisal' | 'template' | 'image';
}

const DeleteModalContentAppraisal: React.FC<DeleteModalContentProps> = ({ close, deleteComps, label, type }) => {
  const enumToUse = type === 'template' ? DeleteTemaplateEnum : type === 'image' ? DeleteImageEnum : DeleteAppraisalEnum;
  return (
