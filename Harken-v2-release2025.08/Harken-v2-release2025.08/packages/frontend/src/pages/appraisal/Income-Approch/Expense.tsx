...
export const Expense = ({
  ...
  const handleAnnualAmountChange = (i: number, event: string) => {
    ...
    let divide: any =
      effectiveGrossIncome !== 0 ? (events / effectiveGrossIncome) * 100 : 0;
    divide = divide.toFixed(2);
    setFieldValue(`operatingExpenses.${i}.percentage_of_gross`, divide);

    const divide_sf =
      values.total_sq_ft && values.total_sq_ft > 0
        ? events / values.total_sq_ft
        : 0;
        ...
        values.operatingExpenses.map((operatingExpenses: any) => {
          ...
          const divide =
            effectiveGrossIncome !== 0
              ? annualAmount / effectiveGrossIncome
              : 0;
          return {
            ...
            extra_id: operatingExpenses.extra_id,
          };
        });
  ...
  useEffect(() => {
    if (values?.operatingExpenses) {
      const sum = values.operatingExpenses
        .map((ele) => {
          const val =
            typeof ele.annual_amount === 'string'
  ...
  const totalPercentage = useMemo(() => {
    ...
    const currentOtherIncomeTotal = values.otherIncomeSources?.reduce(
    ...
    const effectiveGrossIncome = (totalAnnualIncomess || 0) + currentOtherIncomeTotal - (Number(values.vacant_amount) || 0);
    return values?.operatingExpenses?.reduce((sum, item) => {
      const annualAmount = parseFloat((item.annual_amount || '0').toString().replace(/,/g, '')) || 0;
      const percentage = effectiveGrossIncome !== 0 ? parseFloat(((annualAmount / effectiveGrossIncome) * 100).toFixed(2)) : 0;
      return sum + percentage;
    }, 0) || 0;
  }, [values.operatingExpenses, values.otherIncomeSources, totalAnnualIncomess, values.vacant_amount]);
  return (
    <>
      ...
      <Typography
        variant="h6"
        component="p"
        className="text-xs font-bold montserrat-font-family mt-0.5 mb-3 px-2"
      >
        EXPENSE
      </Typography>
      ...
      >
        <Grid
          ...
          spacing={2}
          columns={13}
          className="mt-[10px] px-2 mx-0 py-3 w-full"
        >
          <Grid
            item
            xs={2}
            className="pl-0 mb-[25px] font-bold text-[11px]"
          >
            OPERATING EXPENSES
          </Grid>
          <Grid
        ...
        <Grid container spacing={2} columns={13} className="px-2 mx-0 w-full">
          <Grid item xs={2} className="pt-[7px] pl-0">
        ...
        <Grid
          ...
          spacing={2}
          columns={13}
          className="mt-[0px] px-2 mx-0 w-full"
        >
          <Grid item xs={2} className="pt-[7px] pl-0">
            <div className="mt-3 mb-2 font-bold text-[11px]">EXPENSE NOTES</div>
          </Grid>
          <Grid item xs={14} className="p-0">
            <TextEditor
