...
const IncomeApprochSidebar: React.FC<any> = ({ matchName }: any) => {
  ...
  useEffect(() => {
    if (!isFetching && clientApproch?.data) {
      setTimeout(() => {
        setNavigationLoader(false);
      }, 600);
    }
  }, [isFetching, clientApproch?.data]);

  // Fallback to hide loader after maximum wait time
  useEffect(() => {
    if (navigationLoader) {
  ...
  const mapValuesToParams = (values: any) => {
    console.log(
      'IncomeApprochSidebar - values.operatingExpenses:',
      values.operatingExpenses
    );
    const operatingExpenses =
      values.operatingExpenses?.map((ele: any) => {
        console.log('IncomeApprochSidebar - Operating expense element:', ele);
        console.log('IncomeApprochSidebar - ele.extra_id:', ele.extra_id);
        return {
          name: ele.names,
          ...
          percentage_of_gross: (() => {
            const annualAmount = ele.annual_amount
              ? parseFloat(ele.annual_amount.toString().replace(/,/g, ''))
              : 0;

            // Calculate current other income total from the actual array
            const currentOtherIncomeTotal =
              values.otherIncomeSources?.reduce(
                (total: number, source: any) => {
                  return total + (parseFloat(source.annual_income) || 0);
                },
                0
              ) || 0;

            const effectiveGrossIncome =
              (values.total_annual_income || 0) +
              currentOtherIncomeTotal -
              (Number(values.vacant_amount) || 0);
            return effectiveGrossIncome !== 0
              ? parseFloat(
                  ((annualAmount / effectiveGrossIncome) * 100).toFixed(2)
                )
              : 0;
          })(),
    ...
    console.log(
      'IncomeApprochSidebar - Final operatingExpenses:',
      operatingExpenses
    );
    ...
    const income_source =
      values.incomeSources?.map((ele: any) => {
        return {
          ...
          bed: ele.rent_bed
            ...
            : 0,
          unit: ele.unit
            ? parseFloat(ele.unit.toString().replace(/,/g, ''))
            : 0,
    ...
    const other_income_source =
      values.otherIncomeSources
        ?.filter((ele: any) => {
          // Only include rows that have at least one meaningful value
          return (
            ele.type?.trim() || 
            (ele.monthly_income && Number(ele.monthly_income) > 0) ||
            (ele.annual_income && Number(ele.annual_income) > 0) ||
            (ele.square_feet && parseFloat(ele.square_feet.toString().replace(/,/g, '')) > 0) ||
            ele.comments?.trim()
          );
        })
        ?.map((ele: any) => {
          console.log('IncomeApprochSidebar - Other income element:', ele);
          return {
            type: ele.type || '',
            monthly_income: Number(ele.monthly_income) || 0,
            annual_income: Number(ele.annual_income) || 0,
    ...
    return {
      ...
      total_oe_gross: (() => {
        // Calculate current other income total from the actual array
        const currentOtherIncomeTotal =
          values.otherIncomeSources?.reduce((total: number, source: any) => {
            return total + (parseFloat(source.annual_income) || 0);
          }, 0) || 0;
        ...
        return (
          values?.operatingExpenses?.reduce((sum: any, item: any) => {
            const annualAmount =
              parseFloat(
                (item.annual_amount || '0').toString().replace(/,/g, '')
              ) || 0;
            const percentage =
      ...
      monthly_capitalization_rate:
        Number(values.monthly_capitalization_rate) || 0,
      ...
      annual_capitalization_rate:
        Number(values.annual_capitalization_rate) || 0,
  ...
  const otherIncomeUpdate = updateData?.otherIncomeSources?.map((ele: any) => {
    console.log('IncomeApprochSidebar - API other income data:', ele);
    return {
      type: ele.type,
  ...
  useEffect(() => {
    if (compType === 'land_only' && landType && landSize) {
      ...
      if (!hasApiData && incomeSourceLand.length === 0) {
        ...
        console.log(
          'Setting incomeSourceLand from compType fallback:',
          incomeLandFallback
        );
  ...
  const formInitialValues = {
    incomeSources:
      compType === IncomeApproachEnum.LAND_ONLY
        ? ((!data_zonings || data_zonings.length === 0) && (!clientApproch?.data?.data?.incomeSources || clientApproch?.data?.data?.incomeSources.length === 0) ? [{
            type: landType,
            monthly_income: '',
            annual_income: '',
            rent_sq_ft: '',
  ...
  console.log('formInitialValues.incomeSources:', formInitialValues.incomeSources);
