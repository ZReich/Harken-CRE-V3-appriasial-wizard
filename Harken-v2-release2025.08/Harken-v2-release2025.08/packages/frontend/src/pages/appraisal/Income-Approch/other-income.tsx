...
export const OtherIncome = ({ analysis_type }: any) => {
  ...
  const handleMonthlyIncomeChange = (
    ...
    // Create updated array with new values for calculation
    ...
    calculateEffectiveIncome(calculateTotalAnnualIncome(updatedSources));
  ...
  useEffect(() => {
    if (
      values.otherIncomeSources?.length &&
      values.otherIncomeSources.some(
        (source) => source.type || source.monthly_income || source.annual_income
      )
    ) {
  ...
  return (
    <>
      <Typography
        variant="h6"
        component="p"
        className="text-xs font-bold montserrat-font-family mt-0.5 p-2"
      >
        OTHER INCOME
      </Typography>
      ...
      >
        <Grid
          ...
          spacing={2}
          columns={13}
          className="mt-[10px] px-2 py-3 mx-0 w-full"
        >
          <FieldArray
