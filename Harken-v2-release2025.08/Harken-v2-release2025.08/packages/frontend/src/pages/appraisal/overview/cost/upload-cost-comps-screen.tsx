...
const AppraisalCostUploadCompsScreen = () => {
    ...
    try {
      ...
      if (linkedCompIds.length > 0) {
        try {
          ...
          const updatedCompsCost = linkedComps.map(
            ({ id, ...restComp }: any) => ({
              ...restComp,
              comp_id: id,
              expenses:
          ...
          localStorage.setItem(
            'updatedCompsCost',
            JSON.stringify(updatedCompsCost)
          );
          navigate(`/cost-approach?id=${id}&costId=${costId}`, {
            state: { updatedCompsCost },
          });
        } catch (error) {
          navigate(`/cost-approach?id=${id}&costId=${costId}`);
        }
...
export default AppraisalCostUploadCompsScreen;
