...
import LeaseFilteredMapSideListing from './lease-filtered-map-comps-listing';
...
const LeaseAppraisalMapListingIntegrated: React.FC<
  EvaluationSalesMapListingProps
> = (props) => {
  const location = useLocation();

  const compFilters = props.compFilters || null;
  const currentBounds = props.currentBounds;
  const currentMapZoom = props.mapZoom || null;
  ...
  useEffect(() => {
    if (props.localStorageTrigger && props.localStorageTrigger > 0) {
      console.log(
        '🔄 LEASE INTEGRATED: Detected localStorage trigger:',
  ...
  const handleFullscreenToggle = useCallback(() => {
    console.log(
      '🔄 LEASE APPRAISAL: Fullscreen toggle clicked, current state:',
      isFullscreen
    );
      ...
      {!isFullscreen && (
        <Grid item xs={12} md={6} lg={4}>
          <LeaseFilteredMapSideListing
            ...
            currentBounds={currentBounds}
            mapZoom={mapZoom}
            uploadCompsStatus={props.uploadCompsStatus || null}
...
export default LeaseAppraisalMapListingIntegrated;
