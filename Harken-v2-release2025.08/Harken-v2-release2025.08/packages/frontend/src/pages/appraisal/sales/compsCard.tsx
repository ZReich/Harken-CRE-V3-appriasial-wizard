...
import SalesApproachAdjustmentModal from './salesApproachAdjustmetModal';
import {
  capitalizeWords,
  formatPrice,
        ...
        <div key={item.id} className="bg-slate-100 flex-1">
          ...
          <div className="p-2">
            <div className="flex h-[20px] gap-2 items-center">
              <Link
                to={`/update-comps/${item.comp_id}/${id}/sales/${salesId}`}
                className="flex"
              >
                <Icons.VisibleIcon
                              ...
                              ? item.land_size !== null &&
                                ...
                                ? `${
                                    item.land_dimension === 'ACRE'
                                      ...
                                      : formatNumber(Math.round(item.land_size)) // Round to integer
                                  }`
                                : 'N/A'
                              : isLandSizeAcre
                                ? item.land_size !== null &&
                                  ...
                                  ? `${
                                      item.land_dimension === 'SF'
                                        ? formatNumber(
                                            (item.land_size / 43560).toFixed(3) // Retain three decimal places
                                          )
                                        : item.land_size.toLocaleString('en-US') // Integer for Acre
                                    } AC`
                                  : 'N/A'
                                : isCityState
                ...
                <div key={expense.id} className="subjectPropertyCard h-[20px]">
                  {appraisalData?.comp_adjustment_mode === 'Dollar' ? (
                    <div className="flex">
                      <TextField
                        type="text"
                        className="w-1/2 min-h-[36px] [&_.MuiInputBase-formControl]:rounded-none"
                        value={
                          expense.adj_value
          ...
          <div className="mt-2 px-1 flex flex-col gap-[2px] pb-2">
            <p className="text-xs text-gray-500 !m-0 min-h-[20px] font-medium text-ellipsis selectFixedHeight overflow-hidden whitespace-nowrap">
              <SelectTextField
                className="custom-dropdown"
                value={
                ...
                onChange={(e) => {
                  ...
                  // setValues((oldValues: { tableData: { [x: number]: any } }) => {

                  //   const updatedTableData = [...oldValues.tableData];
                  //   const comp = updatedTableData[index];

                  //   // Update the `compatible` key for the specific row
                  //   const updatedComp = {
                  //     ...comp,
                  //     overall_comparability: selectedValue,
                  //   };
