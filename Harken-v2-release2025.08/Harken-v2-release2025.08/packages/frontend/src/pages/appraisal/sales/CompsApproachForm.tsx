...
import FilterLandMapHeaderOptions from './sales-filter-menu-option-land';
...
const CompsApproachForm: React.FC = () => {
  ...
  const [operatingAllExpensesInitial, setOperatingAllExpensesInitial] =
    useState<any[]>([]);
  const [allAppraisalTpe, setAllAppraisalType] = useState<any[]>([]);
  ...
  const { data } = useGet<any>({
    ...
    config: {
      ...
      onSuccess: async (data: any) => {
        const fetchedAppraisalType = data?.data?.data?.appraisal_type;
  ...
  useEffect(() => {
    const fetchAppraisalData = async () => {
      const endPoint = `appraisals/get/${id}`;

      try {
        const response = await axios.get(endPoint);
        console.log(response, 'responseee');

        setAppraisalType(response.data?.data?.data?.appraisal_type);
  ...
  // useEffect(() => {
  //   if (data) {
  //     const setappraisaltypedata = data?.data?.data?.appraisal_type;
  //     setAppraisalType(setappraisaltypedata);
  //   }
  // }, [data]);
  useEffect(() => {
    console.log('appraidaltype', appraisalType);
    const fetchAppraisalData = async () => {
      // Don't make API call if appraisalType is undefined
      if (!appraisalType) {
        console.log('appraisalType is undefined, skipping API call');
        return;
      }

      const endPoint = `appraisals/sale-comparative-attributes?appraisal_type=${appraisalType}`;

      try {
        const response = await axios.get(endPoint);
        console.log(response, 'responseee');
        const fetchSpecificApprisalData = response.data?.data?.data;
        console.log('fetchSpecificApprisalData', fetchSpecificApprisalData);

        // Check if fetchSpecificApprisalData is an array before filtering
        if (!Array.isArray(fetchSpecificApprisalData)) {
          console.error(
            'fetchSpecificApprisalData is not an array:',
            fetchSpecificApprisalData
          );
          return;
        }

        // Extract items with `default === 1`
...
export default CompsApproachForm;
