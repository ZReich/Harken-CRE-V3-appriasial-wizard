...
const DeleteModalReport: React.FC<ConfirmationModalContentProps> = ({
  close,
  deleteId,
  setArrayAfterDelete,
}) => {
  const { mutate } = useMutate<ResponseType, any>({
    ...
    endPoint: `template/delete/${deleteId}`,
  ...
  return <DeleteModalContentAppraisal close={close} deleteComps={deleteComps} label = {DeleteModalLabelEnum.DELETE_LIST}  type="template" />;
};
