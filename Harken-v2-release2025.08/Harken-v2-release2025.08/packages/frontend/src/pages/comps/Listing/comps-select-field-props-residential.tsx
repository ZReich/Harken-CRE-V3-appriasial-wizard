...
const CompSelectFieldResidential: React.FC<CompSelectFieldProps> = ({
  index,
  col,
  row,
  formik,
  ...
  if (isDropdownField || col === 'building_sub_type') {
    ...
    // Check if the current value exists in the options, otherwise set it as an empty string
    const selectedValue = validOptions.some(
      (option) => option.value === row[col]
    )
      ? row[col]
      : '';
...
export default CompSelectFieldResidential;
