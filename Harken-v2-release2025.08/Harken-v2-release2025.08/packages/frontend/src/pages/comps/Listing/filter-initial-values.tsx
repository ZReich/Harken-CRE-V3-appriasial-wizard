...
export const getRequiredColumns = (activeType: any, comps: any) => {
  ...
  if (activeType === 'land_only' && checkType === 'leasesCheckbox') {
    ...
    storedActiveType?.toLowerCase() === 'residential' ||
    activeType?.toLowerCase() === 'residential'
  ) {
    baseColumns = [
      'street_address',
      'city',
  ...
  // Handle "Type My Own" for land_type
  const hasCustomLandType = Array.isArray(comps)
    ? comps.some((row) => row.land_type === 'Type My Own')
    : false;

  if (hasCustomLandType) {
    const landTypeIndex = baseColumns.indexOf('land_type');

    if (landTypeIndex !== -1 && !baseColumns.includes('land_type_custom')) {
      baseColumns.splice(landTypeIndex + 1, 0, 'land_type_custom');
    }
  } else {
    baseColumns = baseColumns.filter((col) => col !== 'land_type_custom');
  }
  ...
  if (activeType === 'residential') {
    ...
    // Ensure building_sub_type is always required for residential
    if (!baseColumns.includes('building_sub_type')) {
      const buildingTypeIndex = baseColumns.indexOf('building_type');
      if (buildingTypeIndex !== -1) {
        baseColumns.splice(buildingTypeIndex + 1, 0, 'building_sub_type');
      }
    }
  }
