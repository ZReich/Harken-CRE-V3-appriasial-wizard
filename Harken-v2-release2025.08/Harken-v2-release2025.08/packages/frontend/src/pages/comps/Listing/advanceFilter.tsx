...
const AdvanceFilter: React.FC<MapFilterProps> = ({
  isOpen,
  setIsOpen,
  onApplyFilter,
  passDataCheckType,
  ...
  passDataCheckType = localStorage.getItem('checkType');
  ...
  const handleSubmit = async (value: FormData) => {
    ...
    const capeRateMinValue = localStorage.getItem('cap_rate_min')
      ? localStorage.getItem('cap_rate_min')!.replace('%', '')
      : value.cap_rate_min
        ? value.cap_rate_min.replace('%', '')
        : '';
    ...
    const params: Omit<FilterComp, 'search'> = {
      ...
      building_sf_min:
        ...
        passDataCheckType === 'leaseCheckBox'
          ? null
          : buildingMin ?? null,
      building_sf_max:
        passDataCheckType === 'leasesCheckbox' ||
    ...
    // Save cap_rate values to localStorage - use empty string when null/undefined
    const capRateMinForStorage =
      params.cap_rate_min !== null && params.cap_rate_min !== undefined
        ? params.cap_rate_min.toString().replace(/%/g, '')
        : '';
    const capRateMaxForStorage =
      params.cap_rate_max !== null && params.cap_rate_max !== undefined
        ? params.cap_rate_max.toString().replace(/%/g, '')
        : '';

    localStorage.setItem(
      'cap_rate_min',
      sanitizeInputPercentage(capRateMinForStorage)
    );
    localStorage.setItem(
      'cap_rate_max',
      sanitizeInputPercentage(capRateMaxForStorage)
    );
  ...
  useEffect(() => {
    ...
    // Don't clear localStorage if we have compFilters (coming from view page)
    if (shouldClear && !compFilters) {
      ClearAdditionalStorage();
    }
  }, [location, compFilters]);

  useEffect(() => {
    const selectedOptions = localStorage.getItem('property_type');
    const stateFromStorage = localStorage.getItem('state');
    const stateFromFilters = compFilters?.state;

    if (selectedOptions) {
      setPropertyType(selectedOptions);
    }

    // Set state from localStorage or compFilters
    const stateToSet = stateFromStorage || stateFromFilters;
    if (stateToSet) {
      setState(stateToSet);
    }
  }, [compFilters?.state]);
  ...
  const propertyTypesFromFilters =
    setSelectedOptions.length === 0 ? null : compFilters?.propertyType || null;

  const citiesFromFilters =
    compFilters?.city && compFilters?.city.length > 0
      ? compFilters?.city.map((cityName: string) => ({ city: cityName }))
      : null;

  // Update selectedCities when citiesFromFilters changes (only on initial load)
  useEffect(() => {
    if (
      citiesFromFilters &&
      citiesFromFilters.length > 0 &&
      selectedCities.length === 0
    ) {
      setSelectedCities(citiesFromFilters.map((city: City) => city.city));
    }
  }, [citiesFromFilters]);
  return (
    <>
                  ...
                  <Form>
                    ...
                    <div className="p-5 overflow-y-auto max-h-[90vh]">
                      <div className="grid grid-cols-3 gap-5 items-end">
                        ...
                        >
                          <div className="w-full searchTypeSelectFilter">
                        ...
                        <div className="w-ful">
                          <SelectTextField
                            ...
                            value={
                              ...
                              // compFilters?.state ||
                              (usaStateOptions.length > 0
                                ? usaStateOptions[0].value
                                : '')
                            }
                            ...
                            <AdvanceFilterMultipleSelectCheckmarks
                              ...
                              compFilters={{
                                ...
                                city:
                                  ...
                                  compFilters?.state
                                    ? compFilters.city
                                    : [], // ⬅️ Clear cities if states don’t match
                              }}
                              onChange={handleCityChange}
                              usaCity={
                                localStorage.getItem('state') ? usaCity : []
                              }
                              value={selectedCities.map((cityName: string) => ({
                                city: cityName,
                              }))}
                              label="Commercial"
                      ...
                      >
                        ...
                        <DatePickerComp
                          ...
                          value={
                            ...
                            // :
                            moment(localStorage.getItem('end_date')) ||
                            // compFilters.end_date ||
                            null
                          }
                        />
                        ...
                        <StyledFieldFilter
                          ...
                          onChange={(
                            ...
                            let input = sanitizeInputLandSize(e.target.value);
                            if (localStorage.getItem('selectedSize') === 'AC')
                          ...
                          value={
                            ...
                            // compFilters?.land_sf_min ||
                            ''
                          } // Use compFilters value as fallback
                        />
                        ...
                        <StyledFieldFilter
                          ...
                          value={localStorage.getItem('land_sf_max') || ''}
