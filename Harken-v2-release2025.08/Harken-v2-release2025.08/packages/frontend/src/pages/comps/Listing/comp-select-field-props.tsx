...
const CompSelectField: React.FC<CompSelectFieldProps> = ({
  ...
  // Debug: log every column rendered
  ...
  // Render Dropdowns
  // inside CompSelectField, keep everything else as you have it

  if (isDropdownField || col === 'building_sub_type') {
    let options;

    if (col === 'building_sub_type') {
      // 👇 Read activeType once
      const activeType = (
        localStorage.getItem('activeType') || ''
      ).toLowerCase();

      if (activeType === 'residential') {
        // 🔹 Residential side → always use residentialOptions
        options = residentialOptions;
      } else {
        // 🔹 Commercial / other side → keep old behavior (dependent on building_type)
        options = subZoneOptions[index] || [];
      }
    } else if (col === 'basement') {
      options = Array.isArray(BasementOptions) ? BasementOptions : [];
    } else if (col === 'topography') {
      options = topographyOptions || [];
    } else if (col === 'lot_shape') {
    ...
    let selectedValue = validOptions.some(
    ...
    if (
      col === 'lease_rate_unit' &&
      (!row[col] || selectedValue === '') &&
      validOptions.length > 0
    ) {
      selectedValue = validOptions[0].value;
      setTimeout(() => {
        handleFieldChange(index, col, validOptions[0].value);
      }, 0);
    }
