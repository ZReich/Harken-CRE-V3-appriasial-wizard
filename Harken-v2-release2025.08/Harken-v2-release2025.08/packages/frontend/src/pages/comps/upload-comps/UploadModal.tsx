...
const UploadModal = ({
  ...
  onClose,
  onUploadSuccess,
  completedComps = new Set(),
}: UploadModalProps) => {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [isUploading, setIsUploading] = useState(false);
  const [uploadProgress, setUploadProgress] = useState(0);
  const [processedFiles] = useState<Set<string>>(completedComps);
  ...
  const handleUpload = async () => {
    ...
    if (processedFiles.has(fileKey)) {
      toast.info('This file has already been processed successfully.');
      return;
    }
    ...
    try {
      ...
      if (response?.data?.data?.statusCode === 200) {
        ...
        setTimeout(() => {
          ...
          onUploadSuccess(response?.data?.data?.data?.properties);
          onClose();
        }, 500);
      } else {
        uploadCompleted = true;
