...
function EvaluationLeaseCompCard({
  ...
  const getLabelFromValue1 = (value: any) => {
    console.log('getLabelFromValue1 called with:', value, 'propertyTypeOptions:', propertyTypeOptions);
    ...
    // If no match, return the value as is
    return value;
  };
  return (
    <>
      <div className="flex flex-col w-[15.5%]">
        ...
        <div key={item.id} className="bg-slate-100 flex-1">
          ...
          <div className="p-2">
            ...
            <p className="text-gray-500 text-xs font-medium overflow-hidden whitespace-nowrap text-ellipsis pb-1">
              {console.log('Zoning data:', item?.zonings, 'Zone value:', item?.zonings?.[0]?.zone)}
              {getLabelFromValue1(item?.zonings?.[0]?.zone) || 'No zoning data'}
            </p>
          </div>
