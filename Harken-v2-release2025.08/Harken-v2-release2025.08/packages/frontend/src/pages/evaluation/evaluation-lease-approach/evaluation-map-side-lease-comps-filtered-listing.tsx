...
const EvaluationLeaseFilteredMapSideListing: React.FC<CompsListingMapProps> = ({
  ...
  const compsLength =
    ...
    localStorage.getItem('compsLengthleaseevaluation');
  ...
  const handleSubmit = async () => {
    console.log('handleSubmit called in correct file');
    console.log(
      'values.salesCompQualitativeAdjustment:',
      (values as any).salesCompQualitativeAdjustment
    );
      ...
      if (response?.data) {
        ...
        const updatedComps = comps.map(({ id, ...restComp }: any) => ({
          ...
          quantitativeAdjustments:
            (values as any).salesCompQualitativeAdjustment?.length > 0
              ? (values as any).salesCompQualitativeAdjustment.map(
                  (exp: any) => ({
                    ...exp,
                    adj_value: 'similar',
                    comparison_basis: 0,
