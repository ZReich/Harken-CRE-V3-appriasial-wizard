...
import EvaluationLeaseFilteredMapSideListing from './evaluation-map-side-lease-comps-filtered-listing';
import { IComp, FilterComp } from '@/components/interface/header-filter';
import { Box, Grid } from '@mui/material';
import { useLocation, useSearchParams } from 'react-router-dom';
import { toast } from 'react-toastify';

interface EvaluationLeaseMapListingProps {
  typeFilter: string;
  typeProperty: any;
...
const EvaluationLeaseMapListingIntegrated: React.FC<
  EvaluationLeaseMapListingProps
> = (props) => {
  ...
  console.log('bahjsgdahjsd', props);
  ...
  const handlePropertyHover = useCallback((propertyId: number | null) => {
    console.log('🏠 LEASE: Setting hovered property ID:', propertyId);
  ...
  const handleFullscreenToggle = useCallback(() => {
    console.log('🔄 LEASE: Toggling fullscreen mode');
  ...
  useEffect(() => {
    console.log('🏠 LEASE: Hovered property ID changed to:', hoveredPropertyId);
  }, [hoveredPropertyId]);

  // Clear hover state when component unmounts or data changes
  useEffect(() => {
    return () => {
      console.log('🏠 LEASE: Clearing hover state on cleanup');
  ...
  useEffect(() => {
    if (props.localStorageTrigger && props.localStorageTrigger > 0) {
      console.log(
        '🔄 LEASE: LocalStorage trigger detected, value:',
        props.localStorageTrigger
      );
      console.log(
        '🔄 LEASE: Cluster API call will be handled by GoogleMapLocationComps'
      );
    ...
    <Grid container spacing={2} sx={{ height: 'calc(100vh - 139px)' }}>
      ...
      {!isFullscreen && (
        <Grid item xs={12} md={6} lg={4}>
          <EvaluationLeaseFilteredMapSideListing
            {...props}
...
export default EvaluationLeaseMapListingIntegrated;
