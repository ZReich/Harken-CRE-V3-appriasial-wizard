...
export const EvaluationOtherIncome = ({
  analysis_type,
  onOtherIncomeChange,
}: any) => {
  ...
  const handleAnnualIncomeChange = (
    ...
    const newTotal = calculateTotalAnnualIncome(values.otherIncomeSources);
    if (onOtherIncomeChange) {
      onOtherIncomeChange(newTotal);
    }
  ...
  const handleMonthlyIncomeChange = (
    ...
    // Update the other income sources array and calculate new total
    const updatedSources = [...values.otherIncomeSources];
    updatedSources[i] = { ...updatedSources[i], annual_income: annualIncome };
    const newTotal = updatedSources.reduce(
      (total, ele) => total + (parseFloat(ele.annual_income) || 0),
      0
    );
    if (onOtherIncomeChange) {
      onOtherIncomeChange(newTotal);
    ...
    if (values.otherIncomeSources?.length) {
      ...
      const totalSFIncome = values.otherIncomeSources.reduce(
        (total, ele: any) => {
          ...
          return (
            total +
            (parseFloat(ele.square_feet.toString().replace(/,/g, '')) || 0)
          );
        },
        0
      );
  ...
  return (
    <>
      ...
      <div
        className="bg-[#f5f5f5] rounded py-1"
        style={{
          width: '100%',
          marginTop: '10px',
          paddingBottom: '10px',
      ...
      >
        ...
        >
          ...
          <Grid item xs={2} xl={2} className="pt-[5px] pl-[15px]">
            <div className="font-bold text-[11px]">
              $
              {values.other_total_monthly_income === 0
                ? '0.00'
                : (values?.other_total_monthly_income?.toLocaleString(
                    undefined,
                    {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
