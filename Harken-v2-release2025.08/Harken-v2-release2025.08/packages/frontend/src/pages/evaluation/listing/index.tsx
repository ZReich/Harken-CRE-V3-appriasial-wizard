...
const EvaluationListingTable: React.FC<CompsListingMapProps> = ({
  ...
  const editSetup = async (id: number, position: string) => {
    localStorage.removeItem('costValuePerUnitForWeightage');
    localStorage.removeItem('indicatedAnnualRangeforWeightage');
    localStorage.removeItem('salesValuePerUnitForWeightage');
    localStorage.removeItem('initialIncomePercentage');
    localStorage.removeItem('initialSalesPercentage');
    localStorage.removeItem('initialCostPercentage');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const fetchedData = result?.data?.data?.evaluations || [];
        // If no data on current page but there are records in total, go to previous page
  ...
  const isPermissionDenied = (mutation?.error as any)?.status === 401 || (mutation?.error?.message?.includes('Permission denied') ?? false);
  console.log(data, 'checkscenario');

  useEffect(() => {
    onPermissionDenied?.(isPermissionDenied);
  }, [isPermissionDenied, onPermissionDenied]);

  // Function to get scenarios for a specific evaluation
  // Function to get scenarios for a specific evaluation
  ...
  // Example usage:
  const scenariosForEvaluation = getScenarios(684);
  console.log(scenariosForEvaluation, 'scenariosForEvaluation');
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} aria-label="caption table">
          ...
          <TableBody>
            {data.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} style={{ textAlign: 'center', padding: '100px 20px 20px 20px' }}>
                  {isPermissionDenied ? (
                    <p className="text-gray-600">
                      Your user profile does not currently permit access to evaluations. If you need access to this tool, please ask one of your account admins to subscribe.
                    </p>
                  ) : (
            ...
            {data.map((row: any) => (
              <TableRow key={row.id}>
                ...
                <TableCell>
                  {[row.street_address, row.city, row.state?.toUpperCase()]
                    .filter(Boolean)
                    .join(', ')}
                </TableCell>
                <TableCell>
