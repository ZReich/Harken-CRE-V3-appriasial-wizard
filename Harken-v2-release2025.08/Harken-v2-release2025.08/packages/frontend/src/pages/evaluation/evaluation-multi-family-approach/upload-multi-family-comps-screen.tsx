...
const MultiFamilyUploadCompsScreen = () => {
  ...
  const handleLinkSelectedComps = async () => {
    ...
    // Check if any comps are selected first
    ...
    try {
      ...
      if (linkedCompIds.length > 0) {
        try {
          ...
          const updatedCompsMultiFamily = linkedComps.map(
            ({ id, ...restComp }: any) => ({
              ...restComp,
              comp_id: id,
              expenses:
          ...
          localStorage.setItem(
            'updatedCompsMultiFamily',
            JSON.stringify(updatedCompsMultiFamily)
          );
          navigate(
            `/evaluation/multi-family-approach?id=${id}&evaluationId=${evaluationId}`,
            {
              state: { updatedCompsMultiFamily },
            }
          );
        } catch (error) {
          navigate(
...
export default MultiFamilyUploadCompsScreen;
