...
import { EvaluationCostApproachFormValues } from '../evaluation-cost-approach/evaluation-cost-approach-subject-property-form';
import {
...
const EvaluationMultiFamilyFilteredCompsList: React.FC<
...
> = ({
  ...
  const compsLength =
    ...
    localStorage.getItem('compsLengthmultifamilyevaluation');
  // const location = useLocation();
  ...
  const calculateCompData = ({ total, weight, comp, appraisalData }: any) => {
    ...
    console.log(updatedAdjustedPsf, 'updatedAdjustedPsf');
  ...
  const {
    ...
    config: {
      ...
      onSuccess(data: any) {
        ...
        const appraisalApproach = data?.data?.data?.evaluation_approaches?.find(
          (approach: any) =>
  ...
  const approachType = localStorage.getItem('checkType');

  const parameterType =
    approachType === 'salesCheck' || !approachType
      ? ListingEnum.SALE
      : ListingEnum.LEASE;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response?.data) {
        ...
        navigate(
          `/evaluation/multi-family-approach?id=${id}&evaluationId=${costId}`,
          {
            state: { updatedComps },
          }
        );
  ...
  const handleCheckboxChange = async (
    ...
    // Call the API with same payload structure
    try {
      const newParameterType =
        selectedType === 'salesCheckbox' ? ListingEnum.SALE : ListingEnum.LEASE;
      const compType =
...
export default EvaluationMultiFamilyFilteredCompsList;
