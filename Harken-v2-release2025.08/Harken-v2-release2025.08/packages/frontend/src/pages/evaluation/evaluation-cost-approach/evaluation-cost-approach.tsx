...
import { defaultOperatingExpenses } from '@/pages/comps/create-comp/SelectOption';
...
export const calculateCompData = ({
  ...
  if (appraisalData.comp_type == 'land_only') {
    ...
    if (
      ...
      baseAdjustedPsf = parseFloat(
        (comp.sale_price / (comp.land_size * ListingEnum.SQFT_ACRE)).toFixed(3)
      );
    }
  }
  ...
  const updatedAdjustedPsf =
    appraisalData.comp_adjustment_mode === 'Dollar'
      ...
      : baseAdjustedPsf + (total / ListingEnum.HUNDRED) * baseAdjustedPsf; // Add percentage of baseAdjustedPsf to itself

  // (total / 100) * price_square_foot + price_square_foot;
  const updatedAverageAdjustedPsf =
    (updatedAdjustedPsf.toFixed(2) * weight) / ListingEnum.HUNDRED;
...
const EvaluationCostApproach: React.FC = () => {
  ...
  console.log(totalPsfValue, totalCostValuation, costIndicatedPsf);
  ...
  console.log(setCostValueModified);
  const [loading, setLoading] = useState(true);
  const [isCompsDataLoaded, setIsCompsDataLoaded] = useState(false);
  ...
  const {
    ...
    config: {
      ...
      onSuccess(data: any) {
        ...
        setComparisonBasis(comparison_basis);
  ...
  const fetchComposData = async (values: any, setValues: any) => {
    try {
      ...
      const evalWeightValue: any = costData * ListingEnum.HUNDRED;
      ...
      setIsCompsDataLoaded(true);
    } catch (error) {
      // Handle the error
      console.error('Error fetching comps data:', error);
      setIsCompsDataLoaded(true); // Set to true even on error to prevent infinite loading
    }
  };
  ...
  values.tableData.forEach((comp: { adjusted_psf: string; weight: any }) => {
    if (comp.adjusted_psf) {
      ...
      totalaveragedadjustedpsf +=
        adjustedPsfValue * (comp.weight / ListingEnum.HUNDRED);
    }
  });
  ...
  const uploadComps = () => {
    ...
    localStorage.setItem('compsLengthcostevaluation', compsLength);

    navigate(`/evaluation/upload-cost-comps?id=${id}&costId=${costId}`, {
      state: {
        operatingExpenses: values.operatingExpenses,
      },
    });
  ...
  if (isLoading || loading || loader || !isCompsDataLoaded) {
    return (
      <>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between h-[50px] w-[100%] xl:pl-[40px] border-0 border-b border-[#eee] border-solid xl:pr-[70px] px-[15px] map-header-sticky">
        ...
        >
          {CreateCompsEnum.COST_COMPS}{' '}
          <span>{costName ? `(${costName})` : ''}</span>
        </Typography>

        <div className="flex items-center gap-2">
          <ErrorOutlineIcon />
          <span className="text-xs">{CreateCompsEnum.VALUE_OF_LAND}</span>
        </div>
      </div>
          ...
          <div className="bg-[#f1f3f4] flex-1 w-full">
            ...
            <div className="mt-2 px-1 flex flex-col gap-[2px] pb-2">
              <p className="italic text-xs text-gray-500 font-medium h-[18px] !m-0 text-ellipsis overflow-hidden whitespace-nowrap invisible">
                Overall Adjustment{' '}
                {/* Contains data of Overall Adjustments in numbers */}
              </p>
