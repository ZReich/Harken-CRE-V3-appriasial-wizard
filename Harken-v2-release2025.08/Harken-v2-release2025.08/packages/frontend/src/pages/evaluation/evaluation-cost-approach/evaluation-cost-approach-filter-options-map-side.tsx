...
const EvaluationCostFilterLandMapHeaderOptions: React.FC<any> = ({
  ...
  console.log('comparison_basis', comparisonBasisView);
  ...
  const [clustersReady, setClustersReady] = useState<boolean>(true);
  ...
  useEffect(() => {
    ...
    // Set clusters not ready when checkType changes to show loader
    setClustersReady(false);
  }, [checkType]);

  // Set clusters not ready when filters change
  useEffect(() => {
    if (sidebarFilters) {
      setClustersReady(false);
    }
  ...
  // Set clusters not ready when search changes
  useEffect(() => {
