...
const EvaluationImage = () => {
  ...
  // Delete image handler (kept for direct use if needed)
  ...
  const openDeleteModal = (imageId: number, uniqueValue: string) => {
    setDeleteTarget({ id: imageId, uniqueValue });
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(
        `evaluations/remove-image?image_id=${deleteTarget.id}&evaluation_id=${id}&field=${deleteTarget.uniqueValue}`
      );
                        ...
                        {extraImage?.id && (
                          <>
                            <IconButton
                              onClick={() =>
                                handleViewImage(
                                  import.meta.env.VITE_S3_URL + extraImage.dir
                                )
                              }
                            >
                              <Icons.VisibleIcon
      ...
      {viewModalOpen && viewImage && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
          <div className="w-7/12 bg-white rounded p-12 relative flex flex-col justify-center items-center min-h-[45vh]">
            ...
            <DialogContent className="p-0" style={{ maxWidth: '90vw', maxHeight: '80vh', overflow: 'hidden' }}>
              <img
                src={viewImage}
                alt="image"
                style={{
