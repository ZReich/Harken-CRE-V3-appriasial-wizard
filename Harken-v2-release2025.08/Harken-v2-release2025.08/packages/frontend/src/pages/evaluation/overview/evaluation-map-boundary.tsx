...
const EvaluationMapBoundary = () => {
  ...
  console.log(polygonData?.length, 'length');
  ...
  console.log(mapTypes, 'mapTypesmapTypes');
  useEffect(() => {
  ...
  const mutation = useMutate<any, any>({
    queryKey: 'logins',
    endPoint: `evaluations/update-map-boundary/${id}`,
    requestType: RequestType.PATCH,
  });
  useEffect(() => {
    setMapType(
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (polygonData.length === 0) {
      navigate(`/evaluation-aerialmap?id=${id}`);
      return;
    }
        ...
        <div className="w-1/3 py-9 px-5 absolute bg-white top-[330px] right-4">
          ...
          <div className="flex justify-center gap-3">
            <Button
              ...
              onClick={() => {
                navigate(`/evaluation-photo-sheet?id=${id}`);
              }}
            >
...
export default EvaluationMapBoundary;
