...
const EvaluationOverviewForm: React.FC<any> = () => {
  ...
  const handleSubmit = (values: any) => {
    ...
    try {
      ...
      console.log('Original state value:', values.state);
      console.log('Available state codes:', Object.entries(UsaStateCodes));
      ...
      const sqFtArray =
        ...
        values.zonings.map((item: any) =>
          item.sq_ft === 0
            ? 0
            : parseInt(String(item.sq_ft).replace(/,/g, ''), 10)
        );
      console.log('Parsed sqFtArray:', sqFtArray);

      const filteredSqFtArray = sqFtArray.filter(
        (sqFt: number) => !isNaN(sqFt)
      );
      ...
      let priceSquareFoot;
      const landAssesment = parseFloat(
        values.land_assessment?.replace('$', '')
      );
      ...
      console.log(
        '📞 About to call mutate with endpoint:',
        `evaluations/update-overview/${id}`
      );
      console.log('📞 Mutation payload preview:', {
      ...
      mutate(
        {
          ...
          state: stateCode ? stateCode.toLowerCase() : values.state || 'tx',
          type: values.type,
          ...
          latitude: values.geometry?.lat ? values.geometry.lat.toString() : '',
          longitude: values.geometry?.lng ? values.geometry.lng.toString() : '',
          ...
          lot_depth: parseFloat(
            (values.lot_depth as string)?.replace(/,/g, '')
          ),
        ...
        },
        {
          onSuccess: async (res) => {
            ...
            console.log(
              'Initial fetched building size:',
              evaluationBuildingSize
            );
