...
// import { ActionTypes, ButtonLabels, ImageOrientation, PropertyTypes, TableHeaders, UniqueValueKeys } from './evaluation-image-enums';
import { ActionTypes,ButtonLabels,ImageOrientation,TableHeaders,UniqueValueKeys } from '@/pages/appraisal/overview/OverviewEnum';
// import { filterTableData, getTableData } from './evaluation-image-config';
import { filterTableData,getTableData } from '@/components/modals/evaluation-approach-config';
interface RowImage {
  url: string;
  uniqueValue: string;
  id: number;
...
const EvaluationImage = () => {
  ...
  const [selectedUniqueValue, setSelectedUniqueValue] = useState<string | null>(null);
    ...
    <EvaluationMenuOptions>
      ...
      {modalOpenCrop && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
          <div className="bg-white h-[650px] w-[800px] rounded p-5 relative flex flex-col justify-center items-center">
            <span
              ...
              {!hide && (
                <div className={`${src ? 'image-loaded' : ''}`}>
                  <DialogContent className="flex items-center justify-center flex-col p-0">
                    ...
                    <Typography variant="h5" className="text-gray-500 text-3xl m-0" gutterBottom>
      ...
      {/* Table */}
      <TableContainer component={Paper} style={{ width: '100%', margin: '20px auto', padding: '0 60px', marginBottom: '82px' }}>
        <Table sx={{ minWidth: 500 }} aria-label="image upload table">
          <TableHead style={{ backgroundColor: '#f0f0f0' }}>
            <TableRow>
              <TableCell style={{ color: 'gray', fontWeight: 'bold' }}>{TableHeaders.IMAGE}</TableCell>
              <TableCell style={{ color: 'gray', fontWeight: 'bold' }} align="center">{TableHeaders.DESCRIPTION}</TableCell>
              <TableCell style={{ color: 'gray', fontWeight: 'bold' }} align="center">{TableHeaders.ORIENTATION}</TableCell>
              <TableCell style={{ color: 'gray', fontWeight: 'bold' }} align="center">{TableHeaders.PAGE}</TableCell>
              <TableCell style={{ color: 'gray', fontWeight: 'bold' }} align="right">{TableHeaders.ACTION}</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {/* Regular images */}
            {filteredData.map((row) => {
              ...
              return (
                <TableRow key={row.id}>
                  <TableCell component="th" scope="row" align="center" style={{ padding: '5px' }}>
                    {matchingImage?.url ? (
                      <div className="max-w-[230px] h-[80px] cursor-pointer m-auto flex object-contain items-center justify-center overflow-hidden">
                        <img
                          ...
                          onClick={() => handleModalOpen(matchingImage.id, row.uniqueValue, row.orientation)}
                        />
                      </div>
                    ) : (
                      <IconButton onClick={() => handleModalOpen(row.id, row.uniqueValue, row.orientation)}>
                        <Icons.AddImageIcon className="large-icon" style={{ fontSize: '50px' }} />
                      </IconButton>
                    )}
                  </TableCell>
                  <TableCell align="center" style={{ padding: '5px' }}>{row.description ?? 'N/A'}</TableCell>
                  <TableCell align="center" style={{ padding: '5px' }}>{row.orientation}</TableCell>
                  <TableCell align="center" style={{ padding: '5px' }}>{row.page}</TableCell>
                  <TableCell align="center" style={{ padding: '5px' }}>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0px' }}>
                      {matchingImage?.id && (
                        <>
                          <Tooltip title="View">
                            <IconButton onClick={() => handleViewImage(matchingImage.url)}>
                              <Icons.VisibleIcon style={{ fontSize: '20px', color: '#0DA1C7' }} />
                            </IconButton>
                          </Tooltip>
                          <Tooltip title="Delete">
                            <IconButton onClick={() => handleDeleteImage(matchingImage.id, matchingImage.uniqueValue)}>
                      ...
                      {extraImage?.dir ? (
                        <div className="max-w-[230px] h-[80px] cursor-pointer m-auto flex object-contain items-center justify-center overflow-hidden">
                          <img
                            ...
                            alt="Extra"
                            onClick={() => handleModalOpen(extraImage.id, UniqueValueKeys.EXTRA_IMAGE, '')}
                          />
                        </div>
                      ) : (
                        <IconButton onClick={() => handleModalOpen(extraImage.id, UniqueValueKeys.EXTRA_IMAGE, '')}>
                          <Icons.AddImageIcon className="large-icon" style={{ fontSize: '50px' }} />
                        </IconButton>
                      )}
                    </TableCell>
                    <TableCell align="center" style={{ padding: '5px' }}>{extraImage.description ?? 'N/A'}</TableCell>
                    <TableCell align="center" style={{ padding: '5px' }}>{extraImage.description}</TableCell>
                    <TableCell align="center" style={{ padding: '5px' }}>{extraImage.description}</TableCell>
                    <TableCell align="center" style={{ padding: '5px' }}>
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0px' }}>
                        {extraImage?.id && (
                          <>
                            <IconButton onClick={() => handleViewImage(extraImage.url)}>
                              <Icons.VisibleIcon style={{ fontSize: '20px', color: '#0DA1C7' }} />
                            </IconButton>
                            <IconButton onClick={() => handleDeleteImage(extraImage.id, UniqueValueKeys.EXTRA_IMAGE)}>
                              <Icons.DeleteIcon style={{ fontSize: '20px' }} />
                            </IconButton>
                          </>
                        )}
      ...
      {viewModalOpen && viewImage && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
          <div className="w-5/12 bg-white rounded p-12 relative flex flex-col justify-center items-center min-h-[60vh]">
            <span
            ...
            <DialogContent className="p-0">
              <img
                ...
                alt="image"
                style={{
                  width: orientation === ImageOrientation.HORIZONTAL ? '100%' : 'auto',
                  height: orientation === ImageOrientation.HORIZONTAL ? 'auto' : '100%',
                }}
              />
            </DialogContent>
          </div>
