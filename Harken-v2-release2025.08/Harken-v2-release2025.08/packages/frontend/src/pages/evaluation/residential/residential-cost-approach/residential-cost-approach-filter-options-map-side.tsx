...
import CostCompsMapListingIntegratedResidential from './comps-map-listing-residential-cost-integrated';
import ResAdvanceFilter from '@/utils/resAdvanceFilter';
export interface Option {
  id: number;
  name: string;
  code: string;
}
const ResidentialCostFilterLandMapHeaderOptions: React.FC<any> = ({
  tableData,
  comparisonBasisView,
}) => {
  ...
  console.log(setSelectedOptions, setSelectPropertyType, sendTypes);
  useEffect(() => {
  ...
  useEffect(() => {
    const handlePageRefresh = () => {
      ...
      localStorage.removeItem('view');
      // Don't remove sortingOrder on page refresh to preserve user selection
      // localStorage.removeItem('sortingOrder');
      if (location.state?.compFilters) {
        // Clear navigation state on refresh
      }
    };
  ...
  }, [checkType, location.state?.compFilters]);
    ...
    <>
          ...
          <Box className="flex">
            ...
            <Box className="ml-3 selectDropdown">
              <CustomSelectSearch
                ...
                value={
                  localStorage.getItem('property_type') || selectedValue || ''
                }
                onChange={(e) => {
                  setPage(1);
                  ...
                  setSelectPropertyType(selectedValue); // Update selectPropertyType to sync with filter
      ...
      <ResAdvanceFilter
        ...
        onApplyFilter={(filter) => {
          setSidebarFilters(filter as any);
        ...
        onPropertyTypeChange={(propertyType) => {
          // Update the menu-options state when property type changes from filter
          setSelectPropertyType(propertyType);
          setSelectedValue(propertyType);
          const valueToStore = propertyType === 'show_all' ? '' : propertyType;
          localStorage.setItem('property_type', valueToStore);
          setSelectType(valueToStore);
          setSidebarFilters(null);
          setHasFilterChanged(true);
      ...
      {selectedButton == 'android' ? (
        ...
        <>
          ...
          <CostCompsMapListingIntegratedResidential
            typeFilter={selectType}
            sidebarFilters={sidebarFilters}
            mapZoom={currentZoom}
            ...
            forceMapUpdate={localStorageTrigger}
          />
        </>
      )}
    </>
...
export default ResidentialCostFilterLandMapHeaderOptions;
