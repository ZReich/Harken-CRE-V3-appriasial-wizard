...
import CompsTable from '@/pages/residential/upload-comps/CompsTable';
import ResidentialUploadCostCompsModal from './residential-upload-comps-pdf';
import axios from 'axios';
import { toast } from 'react-toastify';

const UploadResidentialCostCompsScreenMain = () => {
  ...
  const handleCompSelection = (selectedIndices: number[]) => {
    setSelectedComps(selectedIndices);
  };
  console.group(uploadCompsStatus, pendingNavigate, currentFormikValues);
      ...
      if (linkedCompIds.length > 0) {
        try {
          const getCompsResponse = await axios.post(
            'res-evaluations/get-selected-comps/',
    ...
    if (updatedCompsData) {
      try {
        ...
        for (let i = 0; i < updatedCompsData.length; i++) {
          ...
          await axios.post('resComps/save-extracted-comp', compPayload, {
            headers: { 'Content-Type': 'application/json' },
          });
        }
      } catch (error) {
  ...
  return (
    <Box className="min-h-screen bg-gray-50">
        ...
        {!compsData ? (
          ...
          >
            <Box className="p-2 border-b">
              ...
              >
                Residential Cost Comps Data - Select comps to link
              </Typography>
            </Box>
            <CompsTable
      ...
      <ResidentialUploadCostCompsModal
        ...
        setCompsModalOpen={setUploadModalOpen}
        setCompsData={setCompsData}
        compsData={compsData}
        compsLength={compsData?.length || 0}
...
export default UploadResidentialCostCompsScreenMain;
