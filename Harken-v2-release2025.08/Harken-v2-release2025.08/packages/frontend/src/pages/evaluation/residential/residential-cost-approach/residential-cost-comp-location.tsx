...
const ResidentialGoogleMapAreaInfoCost: React.FC<any> = () => {
  ...
  const costMapId = searchParams.get('costId');
  ...
  const { data: costApproch, refetch: refetchCost } = useGet<any>({
    queryKey: `res-evaluations/get-cost-approach?evaluationId=${id}&evaluationScenarioId=${costMapId}`,
    endPoint: `res-evaluations/get-cost-approach?evaluationId=${id}&evaluationScenarioId=${costMapId}`,
  ...
  const mutation = useMutate<any, any>({
    queryKey: 'costMap',
    endPoint: `res-evaluations/save-cost-area-map/${salesId}`,
    requestType: RequestType.PATCH,
  });
  ...
  const submitAerialMap = async () => {
    ...
    try {
      ...
      toast('Cost Comps Map saved successfully!');
      setLoader(false);
      navigate(
        `/residential/evaluation/cost-approach-improvement?id=${id}&costId=${costMapId}`
      );
    } catch (error) {
  ...
  return isLoaded ? (
    <ResidentialMenuOptions>
      <div className="px-5 py-5 flex justify-between items-center">
        <h1 className="text-2xl font-bold uppercase">Cost Comp Area Map</h1>
      ...
      <div className="w-1/3 py-9 px-5 absolute bg-white top-[330px] right-4">
        ...
        <div className="flex justify-center gap-3">
          <Button
            ...
            onClick={() => {
              navigate(
                `/residential/evaluation/cost-approach?id=${id}&costId=${costMapId}`
              );
            }}
          >
...
export default ResidentialGoogleMapAreaInfoCost;
