...
const EvaluationCapFilteredCompsList: React.FC<CompsListingMapProps> = ({
  typeFilter,
  ...
  const comparisonBasis =
      ...
      localStorage.getItem('comparison_basis') ||
      'SF'; // Default fallback

  const compsLength =
    location.state?.compsLength ||
    localStorage.getItem('compsLengthcapevaluation');
  // const location = useLocation();
  ...
  const fetchComposData = async (values: any, setValues: any) => {
    try {
      ...
      localStorage.setItem('compsLenghtCap', compsArr.length);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const params: paramsType = {
        limit: 10,
        comparison_basis: activeType === 'land_only' ? 'SF' : comparisonBasis,
            ...
            <Box className="round flex items-center gap-4">
              ...
              >
                <input
                  ...
                  id="leasesCheckbox"
                  disabled={checkType === 'leasesCheckbox'}
                />
              ...
              >
                ...
                <Box className="w-44 flex flex-col justify-between">
                  <Box className="flex justify-end">
                    <a href={`/evaluation/cap-comps-view/${item.id}`}>
                      <CommonButton
                        variant="contained"
                        className="text-xs"
                        color="primary"
...
export default EvaluationCapFilteredCompsList;
