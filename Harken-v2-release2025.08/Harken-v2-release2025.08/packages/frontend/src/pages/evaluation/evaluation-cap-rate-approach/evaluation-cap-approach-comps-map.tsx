...
const EvaluationGoogleMapAreaInfoCap: React.FC<any> = () => {
  ...
  const submitAerialMap = async () => {
    console.log('submitAerialMap function called!');
    setLoader(true);
    ...
    try {
      ...
      // === 1. Check CURRENT scenario for multi-family approach ===
      if (currentScenario?.has_multi_family_approach === 1) {
        navigate(`/evaluation/rent-roll?id=${id}&evaluationId=${currentScenario.id}`);
        return;
      }

      // === 2. Check NEXT scenarios for income approach ===
      const nextScenarios = scenarios.filter((s: any) => s.id > Number(capMapId));
      const nextIncome = nextScenarios.find((s: any) => s.has_income_approach === 1);
      if (nextIncome) {
        navigate(`/evaluation/income-approch?id=${id}&IncomeId=${nextIncome.id}`);
        return;
      }

      // === 3. Check NEXT scenarios for cap approach ===
      ...
      // === 4. Check FIRST scenario for sales approach ===
      const salesScenario = scenarios.find((s: any) => s.has_sales_approach === 1);
      if (salesScenario) {
        navigate(`/evaluation/sales-approach?id=${id}&salesId=${salesScenario.id}`);
        return;
      }

      // === 5. Check FIRST scenario for cost approach ===
      const costScenario = scenarios.find((s: any) => s.has_cost_approach === 1);
      if (costScenario) {
        navigate(`/evaluation/cost-approach?id=${id}&costId=${costScenario.id}`);
        return;
      }

      // === 6. Check FIRST scenario for lease approach ===
      const leaseScenario = scenarios.find((s: any) => s.has_lease_approach === 1);
      if (leaseScenario) {
        navigate(`/evaluation/lease-approach?id=${id}&leaseId=${leaseScenario.id}`);
        return;
      }
