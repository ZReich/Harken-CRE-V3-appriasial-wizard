...
import { ListingHeaderEnum, ListingEnum } from '@/pages/comps/enum/CompsEnum';
...
import EvaluationCapFilteredCompsList from './evaluation-cap-filetered-comps-listing';
...
const EvaluationCapFilterLandMapHeaderOptions: React.FC<any> = ({
  // passCompsData,
  tableData,
  comparisonBasisView,
}) => {
  ...
  console.log('checktwooooooo', comparisonBasisView);
  ...
  console.log(setHoveredPropertyId, paginationMeta);
  // Handler for map data updates from cluster-details API
  const handleApiDataUpdate = (
    properties: any[],
    bounds?: any,
    zoom?: number,
    meta?: any
  ) => {
    console.log('📍 Cap Approach Map API data updated:', {
    ...
    // If meta information is provided (like from cluster-details API), store it for pagination
    if (meta) {
      setPaginationMeta({
        totalRecord: meta.totalRecord,
  ...
  const triggerLocalStorageUpdate = () => {
    ...
    console.log(
      '🔄 CAP PARENT: Triggered localStorage update, new value:',
      newValue,
  ...
  const handleToggle1 = (newSelectedButton: string | null) => {
    if (newSelectedButton !== null) {
      console.log('🔄 CAP: SF/AC toggle changed to:', newSelectedButton);
  ...
  const handleInputChange = useCallback(
    debounce((value: string) => {
      console.log('🔄 CAP: Search input changed to:', value);
  ...
  const onSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSearchValuesByfilter(value); // Update immediately for UI
  ...
  return (
    <>
                ...
                {localStorage.getItem('activeType') === 'land_only' ? (
                  <MultipleSelectLandWithAPI
                    onChange={(transformedArray) => {
                      console.log(
                        '🔄 CAP: Land property type changed to:',
                        transformedArray
                      );
                      setPage(1);
                  ...
                  <MultipleSelectWithAPI
                    onChange={(transformedArray) => {
                      console.log(
                        '🔄 CAP: Building property type changed to:',
                        transformedArray
                      );
                      setPage(1);
                      setSelectedOptions(transformedArray); // Update state
            ...
            <Box className="selectDropdown">
              <CustomSelectSearch
                ...
                onChange={(e: SelectChangeEvent<string>) => {
                  console.log('🔄 CAP: Sort by changed to:', e.target.value);
                  setPage(1);
      ...
      />
      {selectedButton == 'android' ? (
        <EvaluationCapFilteredCompsList
          tableData={tableData}
          ...
          <div className="2xl:xl:w-8/12 xl:w-7/12 w-6/12 overflow-hidden">
            <GoogleMapLocation
              ...
              filterParams={{
                ...
                orderByColumn:
                  sortingSettings.sortingType || ListingEnum.DATE_SOLD,
                type: sendTypes,
                search: searchValuesByfilter,
                orderBy: sortingSettings.orderSorting || 'DESC',
                compStatus: localStorage.getItem('checkStatus') || '',
                // Include sidebar filters if they exist
                ...(sidebarFilters
                  ? {
                      ...
                      end_date: sidebarFilters.end_date,
                    }
                  : {}),
              }}
            />
...
export default EvaluationCapFilterLandMapHeaderOptions;
