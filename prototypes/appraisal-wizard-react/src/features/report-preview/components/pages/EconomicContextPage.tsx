...
export const EconomicContextPage: React.FC<EconomicContextPageProps> = ({
  ...
  return (
    ...
    >
      ...
      <table className="w-full text-xs border-collapse mb-4">
        <thead>
          <tr className="bg-slate-100 dark:bg-slate-800">
            <th className="text-left py-2 px-3 font-semibold text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700">
              Indicator
            </th>
            <th className="text-center py-2 px-3 font-semibold text-slate-700 dark:text-slate-200 w-24 border border-slate-200 dark:border-slate-700">
              Current
            </th>
            <th className="text-center py-2 px-3 font-semibold text-slate-700 dark:text-slate-200 w-20 border border-slate-200 dark:border-slate-700">
              Trend
            </th>
          </tr>
        </thead>
        <tbody>
          {indicators.map((indicator, idx) => (
            <tr key={idx} className={idx % 2 === 0 ? 'bg-white dark:bg-slate-900' : 'bg-slate-50 dark:bg-slate-800/50'}>
              <td className="py-2 px-3 border border-slate-200 dark:border-slate-700 font-medium text-slate-800 dark:text-slate-200">
                {indicator.name}
              </td>
              <td className="py-2 px-3 text-center font-semibold text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700">
                {formatRate(indicator.current)}
              </td>
              <td className="py-2 px-3 text-center border border-slate-200 dark:border-slate-700">
                <span
                  className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium ${indicator.trend === 'Rising' ? 'bg-emerald-100 text-emerald-700' :
      ...
      {hasHistory && (
        <>
          <h2 className="text-xs font-semibold text-slate-800 dark:text-slate-200 pb-1 mb-2 border-b border-slate-200 dark:border-slate-700">
            Historical Trends
          </h2>
          <div className="grid grid-cols-2 gap-3 mb-4">
            {displayData.federalFundsRate.history && (
              <div className="rounded-lg p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                <h3 className="text-[10px] font-semibold text-slate-800 dark:text-slate-200 mb-1">Federal Funds Rate</h3>
                <EconomicChart
            ...
            {displayData.treasury10Y.history && (
              <div className="rounded-lg p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                <h3 className="text-[10px] font-semibold text-slate-800 dark:text-slate-200 mb-1">10-Year Treasury</h3>
                <EconomicChart
            ...
            {displayData.inflation.history && (
              <div className="rounded-lg p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                <h3 className="text-[10px] font-semibold text-slate-800 dark:text-slate-200 mb-1">Inflation (CPI)</h3>
                <EconomicChart
            ...
            {displayData.gdpGrowth.history && (
              <div className="rounded-lg p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                <h3 className="text-[10px] font-semibold text-slate-800 dark:text-slate-200 mb-1">GDP Growth</h3>
                <EconomicChart
      ...
      <div className="text-[10px] text-slate-700 dark:text-slate-300 leading-relaxed space-y-2 mb-3">
        <p>
      ...
      {/* Source Attribution */}
      <div className="mt-auto pt-2 text-[9px] text-slate-400 dark:text-slate-500">
