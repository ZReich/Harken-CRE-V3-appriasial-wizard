  ...
  return (
    <div className="border border-light-border dark:border-dark-border rounded-xl overflow-hidden bg-surface-1 dark:bg-elevation-1">
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-3 hover:bg-harken-gray-light dark:hover:bg-elevation-3 transition-colors"
      >
        ...
        {isOpen ? (
          <ChevronUp className="w-4 h-4 text-harken-gray-med dark:text-slate-500 dark:text-slate-400" />
        ) : (
          <ChevronDown className="w-4 h-4 text-harken-gray-med dark:text-slate-500 dark:text-slate-400" />
        )}
      </button>
      {isOpen && (
        <div className="px-4 pb-4 pt-1 border-t border-harken-gray-light dark:border-dark-border">
          {children}
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-2">
        ...
        />
        <div>
          <h3 className="text-lg font-bold text-harken-dark dark:text-white">{guidance.title}</h3>
          <p className="text-sm text-harken-gray-med dark:text-slate-400">Section Guidance</p>
        </div>
      </div>
      ...
      <CollapsibleSection
        ...
        defaultOpen={true}
      >
        <p className="text-sm text-harken-gray dark:text-slate-200 leading-relaxed mt-2">
          {guidance.context}
        </p>
      </CollapsibleSection>
        ...
        >
          <ul className="space-y-2 mt-2">
            {guidance.tips.map((tip, index) => (
              <li key={index} className="flex items-start gap-2 text-sm text-harken-gray dark:text-slate-200">
                <span 
        ...
        >
          <ul className="space-y-2 mt-2">
            {guidance.mistakes.map((mistake, index) => (
              <li key={index} className="flex items-start gap-2 text-sm text-harken-gray dark:text-slate-200">
                <span className="w-1.5 h-1.5 rounded-full bg-harken-error mt-1.5 shrink-0" />
                <span>{mistake}</span>
              </li>
            ))}
          </ul>
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-harken-gray-med dark:text-slate-400 truncate">
