...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  const variants = {
    default: {
      container: 'bg-white border-2 border-dashed border-slate-300',
      iconBg: 'bg-slate-100',
  ...
  return (
    <div className={`${style.container} rounded-xl p-8 text-center`}>
      ...
      {actionLabel && onAction && (
        <button
          onClick={onAction}
          className="inline-flex items-center gap-2 px-6 py-3 bg-[#0da1c7] text-white font-bold rounded-xl hover:bg-[#0b8eb0] transition-colors shadow-md"
        >
