...
function DocumentCard({
  ...
  const getStatusContent = () => {
    switch (doc.status) {
      ...
      case 'error':
        return (
          <div className="flex items-center gap-2 text-harken-error">
  ...
  const colorMap: Record<string, string> = {
    ...
    red: 'bg-accent-red-light text-harken-error border-harken-error/20',
    emerald: 'bg-accent-teal-mint-light text-accent-teal-mint border-accent-teal-mint-light',
    gray: 'bg-harken-gray-light text-harken-dark border-light-border',
    amber: 'bg-amber-100 text-amber-700 border-accent-amber-gold',
  ...
  return (
    <div
      className={`bg-surface-1 dark:bg-elevation-1 border rounded-xl overflow-hidden transition-all duration-300 ${doc.status === 'complete' ? 'border-green-200 dark:border-green-700 shadow-sm' :
        doc.status === 'error' ? 'border-harken-error/20 dark:border-harken-error' :
    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-start gap-3 min-w-0 flex-1">
            {/* Icon */}
            <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${doc.status === 'complete' ? 'bg-green-50 dark:bg-green-900/30' : 'bg-harken-gray-light dark:bg-elevation-1'
              }`}>
              {doc.status === 'classifying' || doc.status === 'extracting' ? (
                <Sparkles className="w-6 h-6 text-[#0da1c7] animate-pulse" />
              ) : doc.status === 'uploading' ? (
                <Loader2 className="w-6 h-6 text-harken-gray-med animate-spin" />
              ) : (() => {
                const IconComponent = ICON_MAP[typeInfo.icon] || HelpCircle;
                return <IconComponent className="w-6 h-6 text-harken-gray dark:text-slate-400" />;
              })()}
            </div>

            {/* File Details */}
            <div className="min-w-0 flex-1">
              <p className="font-medium text-harken-dark dark:text-white truncate">{doc.file.name}</p>
              <p className="text-xs text-harken-gray-med">{formatFileSize(doc.file.size)}</p>
              ...
              {doc.classification && doc.status !== 'classifying' && (
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="text-xs text-harken-gray-med hover:text-[#0da1c7] underline"
                  >
                    Change
                  </button>
                </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {doc.status === 'complete' && (
              <>
                <button
                  ...
                  className="p-1.5 text-harken-gray-med hover:text-[#0da1c7] hover:bg-harken-gray-light rounded-lg transition-colors"
            ...
            <button
              onClick={onRemove}
              className="p-1.5 text-harken-gray-med hover:text-harken-error hover:bg-accent-red-light rounded-lg transition-colors"
              title="Remove"
            >
              <X className="w-4 h-4" />
            </button>
        ...
        {showTypeSelector && (
          <div className="mt-3 p-3 bg-harken-gray-light dark:bg-elevation-1/50 rounded-lg border border-light-border dark:border-dark-border">
            <p className="text-xs font-medium text-harken-gray dark:text-slate-400 mb-2">Select correct document type:</p>
                ...
                .map(type => (
                  <button
                    ...
                    className={`text-left px-3 py-2 rounded-lg text-sm transition-colors ${doc.classification?.documentType === type.id
                      ...
                      : 'bg-surface-1 dark:bg-elevation-1/50 border border-light-border dark:border-harken-gray hover:border-[#0da1c7] hover:text-[#0da1c7]'
                      }`}
                  >
      ...
      {isExpanded && doc.status === 'complete' && doc.extraction?.data && (
        <div className="border-t border-harken-gray-light dark:border-dark-border/50 bg-harken-gray-light dark:bg-elevation-1/50 p-4">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-harken-gray-med dark:text-slate-400 uppercase">Extracted Data</span>
            <span className="text-xs text-accent-teal-mint dark:text-green-400 font-medium flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {Object.entries(doc.extraction.data).map(([field, data]) => {
              ...
              return (
                <div
                  key={field}
                  className="bg-surface-1 dark:bg-elevation-1/50 rounded-lg p-3 border border-light-border dark:border-harken-gray"
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-medium text-harken-gray dark:text-slate-400">{getFieldLabel(field)}</span>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${getConfidenceColorClasses(data.confidence)}`}>
                      {Math.round(data.confidence * 100)}%
                    </span>
                  </div>
                  <p className="text-sm text-harken-dark dark:text-white truncate" title={data.value || ''}>{data.value || '(not extracted)'}</p>
                </div>
              );
            })}
          </div>
      ...
      {isExpanded && doc.status === 'complete' && !doc.extraction?.data && (
        <div className="border-t border-harken-gray-light dark:border-dark-border bg-yellow-50 dark:bg-yellow-900/20 p-4 text-center">
...
export default function DocumentIntakePage() {
  ...
  const sidebar = (
    <div>
      <h2 className="text-lg font-bold text-harken-dark dark:text-white mb-1">Document & Photo Intake</h2>
      ...
      {activeTab === 'documents' && documents.length > 0 && (
        <div className="space-y-3 mb-6">
          <div className="bg-gradient-to-r from-[#0da1c7]/10 to-[#0da1c7]/5 dark:from-[#0da1c7]/20 dark:to-harken-dark/50 rounded-xl p-4 border border-[#0da1c7]/10 dark:border-[#0da1c7]/20">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-harken-dark dark:text-slate-200">Documents</span>
          ...
          {stats.totalFields > 0 && (
            <div className="flex items-center justify-between px-1 text-sm">
              <span className="text-harken-gray dark:text-slate-400">Fields extracted</span>
              <span className="font-semibold text-accent-teal-mint">{stats.totalFields}</span>
            </div>
          )}
      ...
      {activeTab === 'photos' && (
        <div className="space-y-3 mb-6">
          <div className="bg-gradient-to-r from-[#0da1c7]/10 to-[#0da1c7]/5 dark:from-[#0da1c7]/20 dark:to-harken-dark/50 rounded-xl p-4 border border-[#0da1c7]/10 dark:border-[#0da1c7]/20">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-harken-dark dark:text-slate-200">Photos Uploaded</span>
              <span className="text-lg font-bold text-[#0da1c7] dark:text-cyan-400">{stagingPhotos.length + assignedPhotoCount}</span>
            </div>
            {unassignedPhotos.length > 0 && (
              <p className="text-xs text-accent-amber-gold dark:text-amber-400 flex items-center gap-1 mt-2 font-medium">
          ...
          {wizardState.coverPhoto && (
            <div className="flex items-center gap-2 px-1 text-sm text-accent-teal-mint">
              <CheckCircle className="w-4 h-4" />
      ...
      {activeTab === 'documents' && Object.keys(documentsByType).length > 0 && (
        <div className="space-y-2">
          <h3 className="text-xs font-semibold text-harken-gray-med uppercase tracking-wide">Identified Documents</h3>
          {Object.entries(documentsByType).map(([type, docs]) => {
            ...
            return (
              <div
                key={type}
                className="flex items-center gap-3 p-2.5 rounded-lg bg-harken-gray-light dark:bg-elevation-1/50 border border-transparent dark:border-dark-border/50 hover:bg-harken-gray-light dark:hover:bg-elevation-3/50 transition-colors"
              >
                <IconComponent className="w-5 h-5 text-harken-gray dark:text-slate-400" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-harken-dark truncate">{typeInfo.label}</p>
                  <p className="text-xs text-harken-gray-med">{docs.length} file{docs.length > 1 ? 's' : ''}</p>
                </div>
    ...
    >
      <ErrorBoundary
        ...
        fallback={
          <div className="p-8 text-center">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-accent-red-light mb-4">
              <AlertCircle className="w-8 h-8 text-harken-error" />
            </div>
            <h2 className="text-xl font-bold text-harken-dark mb-2">Something went wrong</h2>
            <p className="text-harken-gray mb-6 max-w-md mx-auto">
              The application encountered an error while processing the document intake page.
              please try refreshing the page.
            </p>
            <div className="p-4 bg-harken-gray-light rounded-lg text-left max-w-lg mx-auto overflow-auto max-h-48 mb-6 font-mono text-xs">
      ...
      >
        <div className="animate-fade-in">
            ...
            <>
              ...
              <div className="bg-gradient-to-br from-[#0da1c7]/10 via-[#4db8d1]/5 to-transparent border border-[#0da1c7]/20 rounded-2xl p-6 mb-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-harken-gray dark:text-slate-200/90">
              ...
              <div
                ...
                className={`relative border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all duration-300 mb-6 ${isDragging
                  ...
                  : 'border-light-border dark:border-dark-border hover:border-[#0da1c7] dark:hover:border-[#0da1c7] hover:bg-[#0da1c7]/5'
                  }`}
              >
                <input
                  ref={fileInputRef}
                ...
                <div className={`transition-transform duration-300 ${isDragging ? 'scale-110' : ''}`}>
                  ...
                  <p className="text-sm text-harken-gray-med dark:text-slate-400 mb-3">
                    or click to browse
                  </p>
                  <p className="text-xs text-harken-gray-med dark:text-slate-500 underline-offset-4">
              ...
              {documents.length > 0 && (
                <div className="space-y-3 mb-6">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-semibold text-harken-dark">
                      Uploaded Documents ({documents.length})
                    </h3>
                    {stats.complete === stats.total && stats.total > 0 && (
                      <span className="text-xs text-accent-teal-mint font-medium flex items-center gap-1">
                  ...
                  <div className="space-y-3">
                    {documents.map(doc => (
                      <ErrorBoundary
                        ...
                        fallback={
                          <div className="p-4 bg-accent-red-light border border-harken-error/20 rounded-xl flex items-center gap-3">
                            <div className="p-2 bg-accent-red-light rounded-full">
                              <AlertCircle className="w-5 h-5 text-harken-error" />
                            </div>
                            <div className="flex-1">
                              <h3 className="text-sm font-medium text-harken-error">Error displaying document</h3>
                              <p className="text-xs text-harken-error">
                                {doc.file.name} could not be rendered.
                                <button
                                  onClick={() => removeDocument(doc.id)}
                                  className="ml-2 underline hover:text-harken-error"
                                >
                                  Remove
                                </button>
                              </p>
              ...
              <div className="bg-gradient-to-br from-[#0da1c7]/10 via-[#4db8d1]/5 to-transparent border border-[#0da1c7]/20 rounded-2xl p-6 mb-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-harken-gray dark:text-slate-400">
          ...
          {/* Continue Button */}
          <div className="mt-8 pt-6 border-t border-light-border flex justify-between items-center">
            <button
              onClick={() => navigate('/template')}
              className="px-4 py-2 text-harken-gray hover:text-harken-dark transition-colors"
            >
              ← Back to Templates
            </button>
            <button
