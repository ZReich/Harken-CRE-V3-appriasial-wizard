...
import { Trash2, TrendingUp, AlertTriangle, Calendar, MessageSquare, X } from 'lucide-react';
...
export const InputRow: React.FC<InputRowProps> = ({ 
  ...
  const marketRentTotal = (item.marketRentPerSf || 0) * (item.itemSqFt || 0);
  const gap = marketRentTotal - item.amount;
  const isBelowMarket = gap > 0;

  // AI Generation Handler for this specific line item
  const handleAiDraft = async () => {
    try {
        ...
        let context = "";
        if (variant === 'rental') {
            context = `Type: Rental Income. Size: ${item.itemSqFt} SF. Contract Rent: $${contractRentPerSf.toFixed(2)}/SF. Market Rent: $${item.marketRentPerSf}/SF. Lease Expiry: ${item.leaseExpiry || 'N/A'}.`;
        } else if (variant === 'expense') {
            context = `Type: Operating Expense. Cost/SF: $${costPerSf.toFixed(2)}. % of EGI: ${percentEgi.toFixed(1)}%.`;
        } else {
            context = `Type: ${variant}. Annual Amount: $${item.amount}.`;
        }

        const prompt = `Act as an expert real estate appraiser. Write a concise, professional technical note (max 40 words) for the following line item in an appraisal report.
        Item Name: ${item.name}
        Amount: $${item.amount.toLocaleString()}
        Context: ${context}
        
        Instructions:
        - If it's rent, comment on how it compares to market (below/above/at market).
        - If it's an expense, comment on the reasonableness of the per SF or % EGI metric.
        - Use professional, objective language.
        - Do not use "I think" or "We believe".
        `;
  ...
  const inputContainerClass = "relative bg-slate-100 rounded-lg group-focus-within:bg-white group-focus-within:ring-2 group-focus-within:ring-cyan-400 transition-all duration-200 overflow-hidden border border-transparent group-focus-within:border-cyan-200";
  ...
  return (
    <div className="group relative flex flex-col sm:flex-row items-start sm:items-center gap-3 py-3 border-b border-slate-100 last:border-0 hover:bg-white/50 transition-colors p-2 rounded-xl z-0 hover:z-10">
      ...
      {/* 1. NAME COLUMN - Adjusted to be slightly smaller and allow flex-grow while others are wider */}
             ...
             <div className="flex items-center gap-2 mt-2 ml-1">
                 <div className="bg-white border border-slate-200 rounded px-2 py-1 flex items-center gap-2 shadow-sm">
                    ...
                    <input 
                        ...
                        className="text-[11px] bg-transparent text-slate-600 hover:text-cyan-600 focus:text-cyan-600 outline-none cursor-pointer uppercase font-bold tracking-wide"
                    />
                 </div>
      ...
      {/* 2. RENTAL VARIANT COLUMNS - Expanded for more space */}
      {variant === 'rental' && (
        <div className="flex gap-2 w-full sm:w-auto">
            {/* SF - Expanded from w-20 to w-28 */}
            ...
            {/* Contract Rent $/SF - Expanded from w-24 to w-32 */}
            <div className={`w-32 flex-shrink-0 ${inputContainerClass}`}>
             ...
             {/* Market Rent $/SF - Expanded from w-24 to w-32 */}
            <div className={`w-32 flex-shrink-0 ${inputContainerClass}`}>
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        className={`${inputBaseClass} text-right pl-6 text-cyan-600`}
                        placeholder="0.00"
                    />
                </div>
            </div>

            {/* Annual Income - Expanded from w-32 to w-40 */}
      ...
      {/* 3. STANDARD / OTHER / REIMBURSEMENT - Expanded to w-48 */}
      ...
      {/* 4. EXPENSE & RESERVES VARIANT - Expanded for visibility */}
      {(variant === 'expense' || variant === 'reserve') && (
        <div className="flex gap-3 items-center w-full sm:w-auto">
            {/* ANNUAL AMOUNT - Expanded to w-48 */}
            <div className={`w-48 flex-shrink-0 ${inputContainerClass} bg-white ring-1 ring-slate-200`}>
            ...
            {/* Metrics column - Expanded for readability */}
      ...
      <div className="relative w-full sm:w-48 hidden md:block">
            ...
            <div 
                ...
                className={`${inputContainerClass} cursor-text hover:bg-white hover:shadow-sm hover:ring-1 hover:ring-cyan-200 transition-all`}
            >
                <div className="flex justify-between items-start">
                    <label className={labelClass}>Notes</label>
                    <MessageSquare size={12} className="text-cyan-400 mt-2 mr-2 opacity-50" />
                </div>
            ...
            {isNoteOpen && (
                <>
                    ...
                    <div className="absolute top-0 right-0 w-[600px] z-50 animate-in fade-in zoom-in-95 duration-200 origin-top-right">
                        <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden ring-1 ring-slate-200 p-1">
                            ...
                            <div className="px-2 -mt-4">
                                <RichTextEditor 
                                    ...
                                    onGenerateSnippet={handleAiDraft}
                                />
                            </div>
