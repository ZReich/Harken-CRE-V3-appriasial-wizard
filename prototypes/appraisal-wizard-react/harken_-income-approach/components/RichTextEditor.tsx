...
import { Bold, Italic, Underline, List, AlignLeft, Wand2, Maximize2, Minimize2, Database, FileText, Save, X, Plus, Loader2 } from 'lucide-react';
...
export const RichTextEditor: React.FC<RichTextEditorProps> = ({ label, value, onChange, onGenerateSnippet }) => {
  ...
  const ToolbarButton = ({ icon: Icon, active = false }: { icon: any, active?: boolean }) => (
    <button className={`p-2 rounded-lg transition-all ${active ? 'bg-cyan-100 text-cyan-700' : 'text-slate-500 hover:bg-slate-100 hover:text-cyan-600'}`}>
      <Icon size={18} />
    </button>
  );
  ...
  const ActionButton = ({ icon: Icon, label, onClick }: { icon: any, label: string, onClick?: () => void }) => (
    <button 
      onClick={onClick}
      className="flex items-center gap-2 px-3 py-2 text-xs font-bold uppercase tracking-wide text-slate-600 bg-white border border-slate-200 rounded-lg hover:border-cyan-300 hover:text-cyan-700 transition-all shadow-sm active:scale-95 whitespace-nowrap"
    >
      <Icon size={14} />
      {label}
    </button>
      ...
      <div className={`flex items-center gap-2 p-3 ${isFullScreen ? 'px-6 py-4 bg-white border-b border-slate-200' : 'bg-slate-50 border-b border-slate-100'} overflow-x-auto`}>
        ...
        {onGenerateSnippet && (
           <button 
             ...
             className="text-xs flex items-center gap-1 text-cyan-600 hover:text-cyan-700 font-bold uppercase tracking-wide transition-colors px-2 whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed"
           >
  ...
  return (
    <div className="w-full group mt-6">
      ...
      <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm transition-all duration-300 focus-within:shadow-lg focus-within:ring-2 focus-within:ring-cyan-100 focus-within:border-cyan-300">
        <EditorContent />
      </div>
    </div>
  );
