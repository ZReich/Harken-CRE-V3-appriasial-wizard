...
import { Plus, AlertCircle, Building2, Wallet, PieChart, Info, Settings, ChevronRight, Calculator, Activity, Check, LayoutGrid, Home, Warehouse, Hotel, Store, TrendingUp, DollarSign, CalendarRange, ArrowRightLeft, Layers } from 'lucide-react';
import { IncomeData, ExpenseData, LineItem, FinancialSummary, PropertyMeta, ValuationData } from './types';
...
type ValuationScenario = 'as-is' | 'as-completed' | 'as-stabilized' | 'liquidation';

export default function App() {
  // --- State ---
  const [activeTab, setActiveTab] = useState<'income' | 'sales' | 'cost'>('income');
  const [showGuidance, setShowGuidance] = useState(true);
  const [scenario, setScenario] = useState<ValuationScenario>('as-stabilized');
  ...
  const summary = useMemo<FinancialSummary>(() => {
    ...
    const potentialGrossIncome = potentialGrossRevenue; // Alias
  ...
  const dcfAnalysis = useMemo(() => {
    ...
    const sellingCosts = 0.02; // 2% Sales Cost standard

    let cumulativePV = 0;
    let currentNOI = summary.netOperatingIncome; // Year 1 NOI
    
    // Cash Flows (Year 1 to N)
    ...
    // Reversion (Sale in Year N+1)
    // currentNOI is now Year N+1 NOI
  ...
  // --- AI HANDLERS ---
  const generateDraft = async (section: 'revenue' | 'expenses' | 'valuation') => {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        let prompt = "";

        if (section === 'revenue') {
            prompt = `Act as an MAI designated real estate appraiser. Write a professional "Revenue Methodology" commentary for the Income Approach.
            
            Context:
            - Valuation Scenario: ${scenario.toUpperCase()}
            - Property Type: ${propertyMeta.type}
            - Size: ${propertyMeta.sqFt.toLocaleString()} SF
            - Units: ${propertyMeta.unitCount}

            Financial Data:
            - Potential Base Rent: $${summary.potentialBaseRent.toLocaleString()}
            - Potential Market Rent: $${summary.potentialMarketRent.toLocaleString()}
            - Vacancy Rate Applied: ${incomeData.vacancyRate}%
            - Effective Gross Income (EGI): $${summary.effectiveGrossIncome.toLocaleString()}
            
            Instructions:
            - Discuss the revenue methodology specifically for a ${scenario} valuation.
            - Justify the vacancy rate assumption.
            - Keep it concise (approx 100-150 words).
            `;
        } else if (section === 'expenses') {
             prompt = `Act as an MAI designated real estate appraiser. Write a professional "Expense Methodology" commentary for the Income Approach.

             Property Context: ${propertyMeta.type}, ${propertyMeta.sqFt.toLocaleString()} SF.

             Financial Data:
             - Total Operating Expenses: $${summary.totalExpenses.toLocaleString()}
             - Expense Ratio: ${summary.expenseRatio.toFixed(1)}%
             - Expenses per SF: $${summary.expensesPerSf.toFixed(2)}
             - Reserves for Replacement: $${summary.totalReserves.toLocaleString()}
             
             Instructions:
             - Discuss reasonableness of the expense load for a ${scenario} scenario.
             - Highlight major expense categories.
             - Keep it concise (approx 100-150 words).
             `;
        } else if (section === 'valuation') {
             prompt = `Act as an MAI designated real estate appraiser. Write a professional "Reconciliation and Value Conclusion" for the Income Approach.
             
             Subject Property: ${propertyMeta.type}, ${propertyMeta.sqFt.toLocaleString()} SF.
             Valuation Lens: ${scenario.toUpperCase()}

             Direct Capitalization Indicated Value: $${directCapValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}
             Yield Capitalization (DCF) Indicated Value: $${dcfAnalysis.totalValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}
             Going-In Cap Rate: ${valuationData.marketCapRate}%
             
             Instructions:
             - Reconcile the two values for this ${scenario} scenario.
             - Comment on the capitalization rate selections.
             - Provide a final concluded value.
             `;
        }
        ...
        return "Error generating draft. Please check your connection.";
    }
  };
  ...
  const AddButton = ({ label, onClick }: { label: string, onClick: () => void }) => (
      <button 
        onClick={onClick}
        className="w-full mt-2 py-4 border-2 border-dashed border-slate-300 rounded-xl flex items-center justify-center gap-2 text-slate-500 font-bold hover:border-cyan-400 hover:text-cyan-600 hover:bg-cyan-50/50 transition-all duration-300 group"
      >
          <div className="w-8 h-8 rounded-full bg-slate-100 group-hover:bg-cyan-100 flex items-center justify-center transition-colors">
            <Plus size={18} className="text-slate-400 group-hover:text-cyan-600" />
          </div>
          {label}
      </button>
  );
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA] text-slate-900 pb-20 font-sans">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm/50">
        <div className="max-w-[1600px] mx-auto px-6 h-20 flex items-center justify-between">
            <div className="flex items-center gap-4">
                 ...
                 <div>
                    <h1 className="text-xl font-black tracking-tight text-slate-800">Appraisal Wizard</h1>
                    <div className="flex items-center gap-2 text-xs font-medium text-slate-400">
                        <span>Project #24-001</span>
                        <span>•</span>
            ...
            <div className="hidden md:flex bg-slate-100 p-1 rounded-full">
                <button className="px-5 py-2 rounded-full text-sm font-bold text-slate-500 hover:text-slate-800">Template</button>
                <button className="px-5 py-2 rounded-full text-sm font-bold text-slate-500 hover:text-slate-800">Subject</button>
                <button className="px-5 py-2 rounded-full bg-white shadow-sm text-cyan-700 text-sm font-bold">Analysis</button>
                <button className="px-5 py-2 rounded-full text-sm font-bold text-slate-500 hover:text-slate-800">Review</button>
            </div>
            ...
            <div className="flex items-center gap-4">
                <div className="flex items-center gap-2 bg-slate-100 rounded-lg p-1">
                    <button 
                        ...
                        className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${showGuidance ? 'bg-white shadow text-cyan-700' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Guidance
                    </button>
                    <button 
                ...
                <button className="bg-slate-900 text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:bg-slate-800 transition-colors shadow-lg shadow-slate-900/20">
                    Continue &rarr;
                </button>
            </div>
        </div>
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-8 flex items-start gap-8">
          ...
          <div className="flex-grow space-y-6 min-w-0">
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm flex flex-col lg:flex-row items-center justify-between gap-6">
                 ...
                 {/* Scenario Switcher - Critical for Professional Appraisal */}
                 <div className="flex flex-col gap-1 w-full lg:w-auto">
                    ...
                    <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200">
                        {(['as-is', 'as-completed', 'as-stabilized', 'liquidation'] as const).map((s) => (
                            <button
                                key={s}
                                onClick={() => setScenario(s)}
                                className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-tight transition-all whitespace-nowrap ${scenario === s ? 'bg-white text-cyan-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                            >
                                {s.replace('-', ' ')}
                            </button>
                        ))}
                     ...
                     <div>
                         ...
                         <div className="flex items-center gap-2 group">
                             <input 
                                 ...
                                 className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 font-bold text-slate-700 w-32 focus:ring-2 focus:ring-cyan-500 outline-none transition-all focus:bg-white"
                             />
                             <span className="text-xs font-bold text-slate-400 group-hover:text-cyan-500 transition-colors">SF</span>
                         </div>
                     </div>
                     <div>
                         ...
                         <div className="flex items-center gap-2 group">
                             <input 
                                 ...
                                 className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 font-bold text-slate-700 w-24 focus:ring-2 focus:ring-cyan-500 outline-none transition-all focus:bg-white"
                             />
                             <span className="text-xs font-bold text-slate-400 group-hover:text-cyan-500 transition-colors">Units</span>
                         </div>
                     </div>
                 </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between">
                      ...
                      <div className="text-2xl font-black text-cyan-600">${directCapValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}</div>
                  </div>
                   <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between">
              ...
              {/* INCOME SECTION */}
              <div className="bg-white rounded-3xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-slate-100 overflow-hidden">
                  <div className="p-8 pb-4">
                      ...
                      <div className="mb-10">
                       ...
                       {/* VACANCY BOX - FIXED DARK BACKGROUND */}
                  ...
                  <div className="px-8 pb-8">
                     <RichTextEditor 
                        ...
                        onGenerateSnippet={() => generateDraft('revenue')}
                     />
                  </div>
              </div>
                  ...
                  <div className="p-8 pb-4">
                   ...
                   <div className="px-8 pb-8">
                     <RichTextEditor 
                        ...
                        onGenerateSnippet={() => generateDraft('expenses')}
                     />
                  </div>
              </div>
                      ...
                      <div className="flex items-center justify-between">
                           <div className="flex items-center gap-4">
                               ...
                               <div>
                                   ...
                                   <p className="text-slate-400 text-sm font-medium">Reconciliation: {scenario.toUpperCase()} scenario</p>
                               </div>
                           </div>
                           <div className="text-right">
                  ...
                  <div className="p-8">
                   ...
                   <div className="px-8 pb-8">
                     <RichTextEditor 
                        ...
                        onGenerateSnippet={() => generateDraft('valuation')}
                     />
                  </div>
              </div>
          </div>
           ...
           {showGuidance && (
               <div className="w-80 flex-shrink-0 hidden 2xl:block animate-in slide-in-from-right duration-500">
                   <div className="sticky top-28 space-y-6">
                       
                       <div className="bg-white rounded-3xl p-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-slate-100">
                       ...
                       <div className="bg-cyan-50 border border-cyan-100 rounded-3xl p-6">
                           <div className="flex items-start gap-3">
                               <Info className="text-cyan-600 mt-1 flex-shrink-0" size={20} />
                               <div>
                                   <h4 className="font-bold text-cyan-900 text-sm mb-1">Methodology Tip</h4>
      ...
      <RiskAnalysisModal isOpen={isRiskModalOpen} onClose={() => setIsRiskModalOpen(false)} summary={summary} expenses={expenseData} income={incomeData} propertyMeta={propertyMeta} />
    </div>
  );
}
