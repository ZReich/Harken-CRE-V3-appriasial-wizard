...
function determineRequiredScenarios() {
    ...
    if (propertyStatus === 'proposed') {
        result.push({ id: 1, name: 'As Proposed', nameSelect: 'As Proposed', customName: '', approaches: getDefaultApproachesForScenario('As Proposed'), isRequired: true, requirementSource: 'Proposed development - no existing improvements' });
        result.push({ id: 2, name: 'As Completed', nameSelect: 'As Completed', customName: '', approaches: getDefaultApproachesForScenario('As Completed'), isRequired: true, requirementSource: 'Interagency Guidelines - construction/development' });
        result.push({ id: 3, name: 'As Stabilized', nameSelect: 'As Stabilized', customName: '', approaches: getDefaultApproachesForScenario('As Stabilized'), isRequired: true, requirementSource: 'Interagency Guidelines - income property stabilization' });
    } 
    else if (propertyStatus === 'under_construction') {
        result.push({ id: 1, name: 'As Is', nameSelect: 'As Is', customName: '', approaches: getDefaultApproachesForScenario('As Is'), isRequired: true, requirementSource: 'Current value of partially-complete improvements' });
        result.push({ id: 2, name: 'As Completed', nameSelect: 'As Completed', customName: '', approaches: getDefaultApproachesForScenario('As Completed'), isRequired: true, requirementSource: 'Interagency Guidelines - construction loan' });
        result.push({ id: 3, name: 'As Stabilized', nameSelect: 'As Stabilized', customName: '', approaches: getDefaultApproachesForScenario('As Stabilized'), isRequired: true, requirementSource: 'Interagency Guidelines - post-construction lease-up' });
    }
    else if (propertyStatus === 'recently_completed') {
        result.push({ id: 1, name: 'As Is', nameSelect: 'As Is', customName: '', approaches: getDefaultApproachesForScenario('As Is'), isRequired: true, requirementSource: 'Current market value' });
        ...
        if (occupancyStatus === 'lease_up' || occupancyStatus === 'vacant') {
            result.push({ id: 2, name: 'As Stabilized', nameSelect: 'As Stabilized', customName: '', approaches: getDefaultApproachesForScenario('As Stabilized'), isRequired: true, requirementSource: 'Property not yet at stabilized occupancy' });
        }
    }
    else {
        result.push({ id: 1, name: 'As Is', nameSelect: 'As Is', customName: '', approaches: getDefaultApproachesForScenario('As Is'), isRequired: true, requirementSource: 'Current market value' });
    ...
    if (plannedChanges === 'major' || plannedChanges === 'change_of_use') {
        if (!result.some(s => s.name === 'As Completed')) {
            result.push({ id: result.length + 1, name: 'As Completed', nameSelect: 'As Completed', customName: '', approaches: getDefaultApproachesForScenario('As Completed'), isRequired: true, requirementSource: 'Major renovation/change of use planned' });
        }
        ...
        if (occupancyStatus && occupancyStatus !== 'not_applicable' && !result.some(s => s.name === 'As Stabilized')) {
            result.push({ id: result.length + 1, name: 'As Stabilized', nameSelect: 'As Stabilized', customName: '', approaches: getDefaultApproachesForScenario('As Stabilized'), isRequired: false, requirementSource: 'Recommended for income property after renovation' });
        }
    }
    
    // === RULE 3: Occupancy Adjustments ===
    if (occupancyStatus === 'lease_up' && !result.some(s => s.name === 'As Stabilized')) {
        result.push({ id: result.length + 1, name: 'As Stabilized', nameSelect: 'As Stabilized', customName: '', approaches: getDefaultApproachesForScenario('As Stabilized'), isRequired: true, requirementSource: 'Property currently in lease-up phase' });
    }
    
    // === RULE 4: Loan Purpose Overrides ===
    if (loanPurpose === 'construction') {
        if (!result.some(s => s.name === 'As Completed')) {
            result.push({ id: result.length + 1, name: 'As Completed', nameSelect: 'As Completed', customName: '', approaches: getDefaultApproachesForScenario('As Completed'), isRequired: true, requirementSource: 'Interagency Guidelines - construction loan requirement' });
        }
        if (!result.some(s => s.name === 'As Stabilized')) {
            result.push({ id: result.length + 1, name: 'As Stabilized', nameSelect: 'As Stabilized', customName: '', approaches: getDefaultApproachesForScenario('As Stabilized'), isRequired: true, requirementSource: 'Interagency Guidelines - construction loan requirement' });
        }
    }
    
    if (loanPurpose === 'bridge' && plannedChanges && plannedChanges !== 'none' && !result.some(s => s.name === 'As Completed')) {
        result.push({ id: result.length + 1, name: 'As Completed', nameSelect: 'As Completed', customName: '', approaches: getDefaultApproachesForScenario('As Completed'), isRequired: false, requirementSource: 'Recommended for bridge financing with planned improvements' });
    }
...
function updateTriggerButtonStates(field, value) {
    ...
    document.querySelectorAll(selector).forEach(btn => {
        ...
        if (btnValue === value) {
            ...
            // Add checkmark for property status buttons
            if (field === 'propertyStatus' && !btn.querySelector('.status-check')) {
        ...
        <div class="max-w-5xl mx-auto space-y-6 animate-fade-in">
            ...
            <div class="card">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    ${renderField({type: 'date', label: 'Effective Date of Appraisal', required: true, id: 'effective_date'})}
                </div>
    ...
    scenarios.forEach((scenario, index) => {
        ...
        if (scenarios.length > 1) {
            if (!scenario.isRequired) {
                ...
                html += `<span class="text-xs text-gray-400 italic">Cannot remove required</span>`;
            }
        }
        html += `</div>`;
