...
import { RetailpropertyTypeJson } from '../../comps/comp-form/fakeJson';
...
import { usa_state } from '../../comps/comp-form/fakeJson';
...
const MapFilter: React.FC<MapFilterProps> = ({
  ...
  const handleSubmit = async (value: FormData) => {
    ...
    if (localStorage.getItem('selectedCities_res')) {
      ...
      const cities = parsedData.map((item: { city: string }) => item.city);
      selectedCity = cities;
    } else {
    ...
    if (saveCityParams) {
      ...
      // Also set standard key for API compatibility
      localStorage.setItem('selectedCities', JSON.stringify(selectedCity));
    }
    ...
    // Also set standard keys for API compatibility
    localStorage.setItem('end_date', params.end_date);
    localStorage.setItem('start_date', params.start_date);
    ...
    // Also set standard keys for API compatibility
    localStorage.setItem('building_sf_min', input);
    localStorage.setItem('building_sf_max', input_max_building);
    localStorage.setItem('land_sf_min', input_land_sf_min);
    localStorage.setItem('land_sf_max', input_land_sf_max);
  };
  ...
  useEffect(() => {
    const propertyType = localStorage.getItem('property_type_res');
