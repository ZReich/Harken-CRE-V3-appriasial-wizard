...
import { PropertyOptions } from '../create-comp/SelectOption';
import loadingImage from '../../../images/loading.png';
import CompSelectFieldResidential from '@/pages/comps/Listing/comps-select-field-props-residential';
const ResidentialCompsForm = ({
  handleClose,
  onClose,
  open,
  compsData,
  ...
  const processedCompsData = React.useMemo(() => {
    ...
    const processed = compsData.map((comp: any) => {
      ...
      if (processedComp.state && typeof processedComp.state === 'string') {
        processedComp.state = processedComp.state.toLowerCase();
      }

      // Convert numeric bedroom and bathroom values to strings to match dropdown options
      if (
        processedComp.bedrooms !== null &&
        processedComp.bedrooms !== undefined
      ) {
        processedComp.bedrooms = String(processedComp.bedrooms);
      }

      if (
        processedComp.bathrooms !== null &&
        processedComp.bathrooms !== undefined
      ) {
        processedComp.bathrooms = String(processedComp.bathrooms);
      }

      // Convert numeric fireplace value to string to match dropdown options
      if (
        processedComp.fireplace !== null &&
        processedComp.fireplace !== undefined
      ) {
        processedComp.fireplace = String(processedComp.fireplace);
      }
  ...
  // Update required columns whenever form values change
  useEffect(() => {
  ...
  }, [activeType, formik.values.comps]);
    ...
    Object.entries(customFields).forEach(([key, customKey]) => {
      if (newComp[key] === 'Type My Own') {
        if (!(customKey in newComp)) {
          // Set specific default values for certain fields
          if (
            customKey === 'building_sub_type_custom' ||
            customKey === 'condition_custom'
          ) {
            newComp[customKey] = 'Unknown'; // ✅ Set to "Unknown" for these fields
          } else {
            newComp[customKey] = ''; // ✅ Empty string for other fields
          }
        }
  ...
  useEffect(() => {
    const updatedComps = formik.values.comps.map((comp: any) => {
      ...
      const isValidBuildingType = PropertyOptions.some(
        (option) => option.value === comp.building_type
      );
      const updatedBuildingType = isValidBuildingType ? comp.building_type : '';

      // Create the updated comp while preserving all existing fields
      const updatedComp = {
        ...comp, // ✅ Preserve all existing fields including custom ones
  ...
  }, [formik.values.comps, saleStatus, sizeTypeOptions, PropertyOptions]);
  ...
  const saveComps = async () => {
    ...
    try {
      ...
      const updatedComps = formik.values.comps.map((comp: any) => ({
        ...
        condition_custom:
          comp.condition === 'Type My Own' ? comp.condition_custom : undefined,
        // Ensure `building_sub_type_custom` is removed if `building_sub_type` is not "Type My Own"
        building_sub_type_custom:
          comp.building_sub_type === 'Type My Own'
...
export default ResidentialCompsForm;
