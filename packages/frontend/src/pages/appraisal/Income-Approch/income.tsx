...
export const Income = ({
  ...
  useEffect(() => {
    ...
    if (values.incomeSources?.length) {
      ...
      const effectiveIncome = (totalAnnualIncome * (100 - vaccancy_change)) / 100;
      setFieldValue(`adjusted_gross_amount`, effectiveIncome);
    }
  }, [values.incomeSources, values.comparison_basis, values.total_bed, values.total_unit, values.total_sq_ft, values.vacancy, ID, bool, setTotalAnnualIncomess]);
  ...
  useEffect(() => {
    ...
    if (values.total_annual_income) {
      ...
      const effectiveInc: any = (values.total_annual_income * (100 - vaccancy_change)) / 100;
      setFieldValue(`adjusted_gross_amount`, effectiveInc);
    }
  }, [totalSF, values.vacancy, values.total_annual_income, values.total_sq_ft]);
  ...
  return (
    <>
      ...
      >
        <Grid container spacing={2} columns={13} className="mt-3.5 px-2 w-full">
          <Grid item xs={2} className="pt-0.5 pl-[24px]">
          ...
          <Grid item xs={2} className=" pl-[24px]">
            <div className="mt-2 font-bold text-xs">EFFECTIVE INCOME</div>
          </Grid>
          <Grid item xs={2} className="pt-1 pl-[24px]"></Grid>
          <Grid item xs={2} className="pt-2.5 pl-[24px]">
            <div className="font-bold text-[11px]">
              ${' '}
              {(
