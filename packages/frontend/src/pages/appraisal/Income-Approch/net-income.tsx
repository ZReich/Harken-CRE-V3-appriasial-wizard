...
export const NetIncome = ({
  ...
  // Only reset rates on initial mount, not on every render
  useEffect(() => {
    // Only reset if we're truly on initial mount (no existing values)
    const hasExistingValues = values.monthly_capitalization_rate || 
  ...
  console.log(unitMarketRate, bedMarketRate, bedHighRate);
  const total =
    ...
    totalExpenses;
  // incomeTotal = 2632536;
  // Main calculation effect - only recalculate when relevant values actually change
  useEffect(() => {
    // Don't reset calculations unnecessarily
    if (
    ...
    // Perform calculations only when we have valid values and total
    if (values.monthly_capitalization_rate && values.monthly_capitalization_rate !== '0' && total) {
      const event = {
        target: {
    ...
    if (values.annual_capitalization_rate && values.annual_capitalization_rate !== '0' && total) {
      const event = {
        target: {
    ...
    if (values.high_capitalization_rate && values.high_capitalization_rate !== '0' && total) {
      const event = {
        target: {
  ...
  return (
    <>
      ...
      <div style={{ width: '100%', marginTop: '10px', paddingBottom: '100px' }}>
        <Grid
          container
          spacing={2}
          columns={13}
