...
const GoogleMapLocationSalesAppraisals: React.FC<SendTypeItem> = ({
  GoogleData,
  ...
  const viewListingItem = (id: number) => {
    ...
    navigate(`/sales-comps-view/${id}/${check}`, {
      state: {
        ...
        if (clusters && Array.isArray(clusters) && clusters.length > 0) {
          ...
          const processedClusters = clusters.map((cluster: any) => {
            ...
            return {
              ...
              comparisonBasis: finalComparisonBasisView || 'SF',
            };
          });

          setClusters(processedClusters);

          // Always keep US center and zoom 5 - ignore cluster distribution
          if (useWorldBounds && !calculatedZoom) {
            setCalculatedZoom(5);
            setCalculatedCenter(defaultCenter);
            console.log('🗺️ Keeping fixed US center and zoom level 5');
          }
    ...
    if (GoogleData && GoogleData.length > 0) {
      const locationData: LocationDataItem[] = GoogleData.map((elem, index) => {
        ...
        return {
          ...
          comparisonBasisView: finalComparisonBasisView || 'SF',
          comparisonBasis: finalComparisonBasisView || 'SF',
          comparisonBasi: finalComparisonBasisView || 'SF',
        };
      });
...
export default GoogleMapLocationSalesAppraisals;
