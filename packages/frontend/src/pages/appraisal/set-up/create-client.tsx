...
const ClientForm = ({ handleCloseModal }: any) => {
        ...
        onSubmit={async (values, actions) => {
          ...
          try {
            ...
            if (
              ...
              responseMessage.message &&
              window.location.pathname.includes('update-appraisal-set-up')
            ) {
              handleCloseModal();
            }
            else if (
              responseMessage.message &&
              window.location.pathname.includes('appraisal-set-up')
            ) {
              handleCloseModal();
            }
            else {
                  ...
                  <>
                    ...
                    <Grid container spacing={2} className="mt-[20px] items-end">
                      <Grid item xs={6} className="relative googleAddressField">
                        ...
                        <div className="px-2">
                          <div
                            className={`google absolute w-4 h-4 inline-block bg-contain bg-center bg-no-repeat bg-[url("../../frontend//src/images/google_logo.png")] top-[30px] ${isRawData ? 'right-10' : 'right-0'}`}
                          ></div>
                        </div>
                      </Grid>
                      <Grid item xs={3}>
