...
const OverviewSidebar: React.FC<any> = () => {
  ...
  const handleSubmit = (values: any) => {
    ...
    // const totalUnitCount = propertyUnits.reduce(
    //   (sum: any, unit: { unit_count: any }) => sum + unit.unit_count,
    //   0
    // );
    // const hasMultiFamilyZone = zonings.some(
    //   (ele: { zone: string }) => ele.zone === 'multi_family'
    // );
    // const multiFamilyUnits = zonings
    //   .filter((ele: { zone: string }) => ele.zone === 'multi_family')
    //   .reduce((sum: any, ele: { unit: any }) => sum + ele.unit, 0);

    // if (values.comparison_basis === 'Unit') {
    //   if (hasMultiFamilyZone && totalUnitCount != multiFamilyUnits) {
    //     toast.error(
    //       'Total Multi-Family "# of Units" is not equal to the total "Unit Count'
    //     );
    //     return;
    //   }
    // }
    mutate(
      {
        ...
        price_square_foot: parseFloat(values.price_square_foot.replace(/,/g, '')) || 0,
        building_size: calculateBuildingSize,
        ...
        under_contract_price: values.type === 'sale' ? parseFloat(
          ...
          ? {
            close_date:
              !values.close_date || values.close_date === 'NaN/NaN/NaN'
                ? null
                : values.close_date || localStorage.getItem('close_date') || null,
          }
          : {
            close_date: null,
          }),
        ...
        traffic_input:
          values.traffic_count === 'inputvalue' ? values.traffic_input.replace(/,/g, '') : null,
        analysis_type: active_comp_type ? values?.analysis_type : null,
        price: values.type === 'non_sale' ? parseFloat((values.price as string)?.replace(/[$,]/g, '')) : null, // price
 ...
 if (isLoading || loader) {
  ...
  const initialValues = {
    ...
    // property_units:
    //   updateData && updateData.property_units?.length > 0
    //     ? propertyUnits
    //     : [
    //         {
    //           id: '',
          ...
          return (
            <>
              ...
              <Form className="w-full lg:pl-[230px] pl-[180px]">
                {[
                  <PropertyDetails validationSchema={validationSchema} />,
                  <Specification
                    passRequired={passRequired}
                    getCompType={getCompType}
                    updateData={updateData}
