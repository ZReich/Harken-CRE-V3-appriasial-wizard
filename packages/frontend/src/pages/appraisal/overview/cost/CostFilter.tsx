...
import CostTableComponent from './CostTableData';
...
const CostFilter: React.FC<any> = ({
  ...
  console.log('city', formValues.city);
  ...
  useEffect(() => {
    ...
    if (savedCities.length > 0) setSelectedCities(savedCities);
  }, []);
                ...
                <Typography id="modal-description" className="mt-4 leading-10">
                  ...
                  <ul className="list-none">
                    <li className="no-decoration">
                      1. It searches only that comps have date sold value.
                    </li>
                    <li className="no-decoration">
                      2. Private comps are visible to only the person who
                      created that comp.
                    </li>
                    <li className="no-decoration">
                      3. It displays land size format according to the size type
                      selected on the overview screen.
                    </li>
                    <li className="no-decoration">
                      4. If the comp type is sale then it searches only for
                      those that comps have a sale price greater than 0.
                    </li>
                    <li className="no-decoration">
                      5. It searches only that comps are linked with the logged
            ...
            <div className="p-5">
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <StyledFieldFilter
                      ...
                      value={
                        formValues.building_sf_min !== null
                          ? Number(formValues.building_sf_min).toLocaleString(
                              'en-US',
                              {
                    ...
                    <StyledFieldFilter
                      ...
                      value={
                        formValues.building_sf_max !== null
                          ? Number(formValues.building_sf_max).toLocaleString(
                              'en-US',
                              {
                                maximumFractionDigits: 2,
                              }
                  ...
                  <div className="flex flex-row gap-4">
                    <StyledFieldFilter
                      ...
                      onKeyDown={(e: React.KeyboardEvent) => {
                        ...
                        if (isBack) {
                          if (
                            ...
                            handleInputChange(handleChange, 'cap_rate_min', '');
                      ...
                      value={formatPercentageInput(formValues?.cap_rate_min)}
                    />
                    ...
                    <StyledFieldFilter
                      ...
                      onKeyDown={(e: React.KeyboardEvent) => {
                        ...
                        if (isBack) {
                          ...
                          // Check if the entire input is selected (select-all case)
                          if (
                            selectionStart === 0 &&
                            selectionEnd >= inputElement.value.length
                          ) {
                            handleInputChange(handleChange, 'cap_rate_max', ''); // Clear the input
                            event.preventDefault(); // Prevent default behavior to avoid popping characters
                          } else {
                            const input =
                              formValues?.cap_rate_max?.toString() || '';
                            const inpArr = Array.from(input);
                            inpArr.pop(); // Pop off the last character
                            handleInputChange(
                              handleChange,
                              'cap_rate_max',
                              inpArr.join('')
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                        ...
                        // Remove percentage symbol and allow numbers with up to 2 decimal places
                        const sanitizedInput = input.replace(/[%|,]/g, '');

                        // Validate that input is a number and allow up to 2 decimal places
                        if (/^\d*\.?\d{0,2}$/.test(sanitizedInput)) {
                          handleInputChange(
                            handleChange,
                            'cap_rate_max',
                      ...
                      value={formatPercentageInput(formValues?.cap_rate_max)}
                    />
                  </div>
                </div>
              </div>
              <Box className="mt-7 flex justify-center items-center w-3/12 mx-auto">
                <CommonButton
                  ...
                  className="w-1/4" // Reduces width to 25% of its parent container
                >
