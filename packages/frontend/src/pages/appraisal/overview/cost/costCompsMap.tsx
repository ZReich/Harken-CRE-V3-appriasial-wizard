...
const GoogleMapAreaInfoCost: React.FC<any> = () => {
  ...
  const { data: costApproch, refetch: refetchCost } = useGet<any>({
    queryKey: `appraisals/get-cost-approach?appraisalId=${id}&appraisalApproachId=${costMapId}`,
    endPoint: `appraisals/get-cost-approach?appraisalId=${id}&appraisalApproachId=${costMapId}`,
  ...
  const mutation = useMutate<any, any>({
    queryKey: 'costMap',
    endPoint: `appraisals/save-cost-area-map/${salesId}`,
    requestType: RequestType.PATCH,
  });
  ...
  const submitAerialMap = async () => {
    ...
    try {
      ...
      navigate(`/cost-approach-improvement?id=${id}&costId=${costMapId}`);
    } catch (error) {
      ...
      <div className="w-1/3 py-9 px-5 absolute bg-white top-[330px] right-4">
        ...
        <div className="flex justify-center gap-3">
          <Button
            ...
            onClick={() => {
              navigate(`/cost-approach?id=${id}&costId=${costMapId}`);
            }}
          >
...
export default React.memo(GoogleMapAreaInfoCost);
