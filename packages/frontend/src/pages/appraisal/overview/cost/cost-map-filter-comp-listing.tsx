...
const CostFilteredMapSideListing: React.FC<CompsListingMapProps> = ({
  ...
  // Effect to call API when filters or page changes
  ...
  */
  ...
  // The data is now provided by the map component when viewport changes or by direct API calls
  ...
  const fetchComposData = async (values: any, setValues: any) => {
    try {
      ...
      const response = await axios.get(
        `appraisals/get-cost-approach?appraisalId=${id}&appraisalApproachId=${salesId}`,
        {}
      );
  ...
  const backToApproach = () => {
    navigate(`/cost-approach?id=${id}&costId=${salesId}`);
  };
...
export default CostFilteredMapSideListing;
