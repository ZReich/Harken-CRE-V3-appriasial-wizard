...
const GoogleMapLocationCostAppraisals: React.FC<SendTypeItem> = ({
  GoogleData,
  ...
  const viewListingItem = (id: number) => {
    ...
    navigate(`/cost-comps-view/${id}/${check}`, {
      state: {
  ...
  const callBothAPIs = useCallback(
    async (map: google.maps.Map, useWorldBounds = false) => {
      if (!map) {
        console.log('🌐 No map available, aborting API calls');
        return;
      }
      ...
      if (useWorldBounds) {
        ...
        east = 180;
        west = -180;
        zoom = calculatedZoom || 5;
        console.log('🌐 Making API calls with world bounds (initial load)');
      } else {
        const bounds = map.getBounds();
        zoom = map.getZoom() || calculatedZoom || 5;

        if (!bounds) {
          console.log('🌐 No bounds available, aborting API calls');
          return;
        }
        ...
        console.log('🌐 Making API calls with current map bounds');
      }
      ...
      try {
        ...
        console.log('🌐 Sales Cluster-details API params:', salesParams);
        console.log('🌐 Clusters API params:', clustersParams);
        ...
        console.log('🌐 Clusters API response:', clustersResponse);
        console.log('🌐 Cluster-details API response:', clusterDetailsResponse);

        // Process clusters API response
        const clusters = clustersResponse.data?.data?.clusters || [];

        if (clusters && Array.isArray(clusters) && clusters.length > 0) {
          console.log('🌐 Processing', clusters.length, 'clusters from API');
          ...
          // Don't auto-adjust viewport - keep US center and zoom 5
          if (useWorldBounds && !calculatedZoom) {
            setCalculatedZoom(5);
            setCalculatedCenter(defaultCenter);
          }
          ...
          console.log('🌐 No clusters found in API response');
          setClusters([]);
          if (onMapClustersChange) {
            onMapClustersChange([]);
          }
        ...
        console.log('🌐 Sales Cluster-details properties:', properties.length);
  ...
  useEffect(() => {
    if (map && filterParams) {
      console.log(
        '🔍 Sales: Filter params changed, making API calls:',
        filterParams
      );
      // Make API calls when filters change
      callBothAPIs(map, false); // Use current map bounds
    }
  }, [filterParams, map, callBothAPIs]); // Dependencies: filterParams, map, and callBothAPIs

  // Debounced API call - waits 1 second after zoom stops
  const debouncedApiCall = useCallback(
    debounce((map: google.maps.Map) => {
      console.log(
        '🕐 1 second has passed since zoom stopped - showing loader and calling APIs'
      );
  ...
  const handleZoomChange = useCallback(
    (map: google.maps.Map) => {
      ...
      console.log('🔍 Zoom changed to:', zoom, '- starting 1 second timer');
  ...
  const handleMapLoad = useCallback(
    (map: google.maps.Map) => {
      console.log('🗺️ Sales Map loaded');
      setMap(map);
      ...
      // Add drag end listener to trigger API calls when map is panned
  ...
  const handleClusterClick = async (cluster: ClusterData) => {
    if (map) {
      console.log('🔍 Sales: Cluster clicked:', cluster);
      ...
      if (cluster.count === 1) {
        ...
        if (property) {
          ...
          console.log(
            '🔍 No matching property found for single cluster, showing red marker only'
          );
        ...
        try {
          if ((cluster as any).bounds) {
            ...
            google.maps.event.addListenerOnce(map, 'bounds_changed', () => {
              ...
              console.log(
                `🔍 Zoomed into cluster with ${cluster.count} properties - InfoWindows will show only on individual marker clicks`
              );
            });
          } else {
      ...
      >
        ...
        {currentZoom < 15 &&
          clusters.map((cluster, index) => {
            const clusterId =
              (cluster as any).id ||
              `sales-cluster-${index}-${cluster.lat}-${cluster.lng}`;
        ...
        {currentZoom > 14 &&
          GoogleData &&
          GoogleData.length > 0 &&
          GoogleData.flatMap((property, index) => {
            ...
            // Check if this property is within any cluster bounds with smaller tolerance
            const isInClusterBounds = clusters.some((cluster) => {
              return (
                Math.abs(propertyLat - cluster.lat) < 0.001 &&
                Math.abs(propertyLng - cluster.lng) < 0.001
              );
            });

            if (!isInClusterBounds) {
              return [];
            }
        ...
        {currentZoom >= 15 &&
          ...
          GoogleData.flatMap((property) => {
            ...
            return [
              <Marker
                key={`red-marker-${property.id}`}
                position={{ lat: propertyLat, lng: propertyLng }}
                onClick={() => {
                  // Show full property InfoWindow when red marker is clicked
                  setMarkerData((prevMarkerData) =>
                    prevMarkerData.map((marker) => ({
                      ...marker,
                      isOpen: marker.propertyId === property.id,
...
export default GoogleMapLocationCostAppraisals;
