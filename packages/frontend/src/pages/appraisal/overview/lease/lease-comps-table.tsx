...
const LeaseForm = ({
  ...
  useEffect(() => {
    const updatedComps = formik.values.comps.map((comp: any) => {
      const isValidLeaseRateUnit = leaseRateUnitOptions.some(
        (option) => option.value === comp.lease_rate_unit
      );

      // Only update lease_rate_unit if it is invalid or missing
      if (!isValidLeaseRateUnit || comp.lease_rate_unit == null) {
        return { ...comp, lease_rate_unit: 'year' };
      }
      return comp;
    });
  ...
  const SetAllData = (newComps: Comp[]) => {
    // Filter only linked items
    const filteredComps = newComps?.filter((comp: any) => comp.link == true);

    const updatedCompss = filteredComps.map(({ comp_id, ...restComp }: any) => {
      return {
        ...restComp,
        comp_id: comp_id, // **Check if id exists before assigning**
    ...
    try {
      const payload = {
        ...
        type:
          ...
          localStorage.getItem('approachType') === 'leaseCheck'
            ? 'lease'
            : 'sale',
      ...
      // Calculate linked and remaining comps
      const totalComps = formik.values.comps.length;
      const linkedComps = formik.values.comps.filter(
        (comp: any) => comp.link === true
      ).length;

      // Show toast message with calculated values
      toast.success(
        `You have linked ${linkedComps} comps, and ${totalComps} comps are saved`
      );
    ...
    <Dialog open={open} onClose={handleClose} maxWidth={false} fullWidth>
      ...
      <DialogContent dividers>
        <Box sx={{ position: 'relative' }}>
          {/* <IconButton
            onClick={() =>
              setCurrentColumnGroup((prev) => Math.max(prev - 1, 0))
            }
            sx={{
              position: 'fixed',
              left: '23%',
              ...
              padding: '5px',
              backgroundColor: '#fff',
              visibility: currentColumnGroup === 0 ? 'hidden' : 'visible',
          ...
          >
            <Icons.BackwardIcon className="icon-backward" />
          </IconButton> */}
                ...
                <TableRow>
                  ...
                  {tableColumns.map((col) => {
                    ...
                    const columnMappings: Record<string, string> = {
                      ...
                      basement_finished_sq_ft: 'Basement Finished',
                    ...
                    <TableRow key={String(row.id)}>
                      ...
                      >
                        ...
                        {/* <Checkbox
                          ...
                          onChange={(e) => {
                            const isChecked = e.target.checked;
                            handleFieldChange(index, 'selected', isChecked);
                            setTimeout(
                              () => handleFieldChange(index, 'link', isChecked),
                              0
          ...
          {/* <IconButton
            onClick={() =>
              setCurrentColumnGroup((prev) =>
                Math.min(prev + 1, totalGroups - 1)
              )
            }
            sx={{
              position: 'fixed',
              ...
              visibility:
                currentColumnGroup === totalGroups - 1 ? 'hidden' : 'visible',
            }}
          >
            <Icons.ArrowForwardIosIcon className="arrow-icon" />
          </IconButton> */}
        </Box>
      </DialogContent>
