...
export const AreaInfo: React.FC = () => {
  ...
  const areaInfoMutation = useMutate<any, any>({
    queryKey: `appraisal`,
    endPoint: `/appraisals/update-areaInfo/${id}`,
  ...
  const { data: areaInfoData } = useGet<any>({
    queryKey: `areaInfo`,
    endPoint: `/appraisals/get-areaInfo/${id}`,
  ...
  const [filtereLeasedData, setFilteredLeaseData] = useState<
    { id: any; name: React.ReactNode }[]
  >([]);
  const [filtereRentData, setFilteredRentData] = useState<
    { id: any; name: React.ReactNode }[]
  >([]);
  ...
  const [hasRentType, setHasRentType] = useState(false);
  const [hasLeaseType, setHasLeaseType] = useState(false);
  useEffect(() => {
    if (
      appraisalData?.data?.data?.appraisal_approaches &&
      !appraisalData.isStale
    ) {
      const updateData = appraisalData.data.data.appraisal_approaches;
      ...
      const rentRoll = updateData.filter(
        (item: { type: string }) => item.type === 'rent_roll'
      );
      setHasRentType(rentRoll.length > 0);
      setFilteredRentData(rentRoll);
    }
  }, [appraisalData?.data?.data?.appraisal_approaches]);
    ...
    try {
      ...
      setTimeout(() => {
        if (hasIncomeType) {
          ...
          navigate(
            `/rent-roll?id=${id}&appraisalId=${filtereRentData?.[0]?.id}`
          );
        } else if (hasSaleType) {
          navigate(
        ...
        <Box className="mt-5 xl:px-[50px] px-[15px]">
          <Grid container spacing={3}>
            <Grid item xs={12}>
              ...
              <Typography
                variant="caption"
                color={cityInfo.length > 1200 ? 'error' : 'textSecondary'}
              >
            ...
            <Grid item xs={12} className="mt-[50px]">
              ...
              <Typography
                variant="caption"
                color={countyInfo.length > 1200 ? 'error' : 'textSecondary'}
              >
          ...
          <div className="flex gap-3 justify-center items-center fixed inset-x-0 bottom-0 bg-white py-5 z-10">
            <Button
              ...
              onClick={() => navigate(`/aerialmap?id=${id}`)}
            >
