// This is a fixed version of the handleFieldChange function
export const createHandleFieldChange = (
  ...
  return (index: number, field: string, value: any) => {
    ...
    // Prevent selecting more than 4 comps
    if (field === 'selected' && value && newTotalSelected > 4) {
      if (!toastDisplayed) {
        // toast.error(
        //   `You have already selected ${compsLength}. You can only add ${4 - compsLength} more.`
        // );
        toastDisplayed = true;
        setTimeout(() => {
    ...
    // Update the field value in Formik with validation
