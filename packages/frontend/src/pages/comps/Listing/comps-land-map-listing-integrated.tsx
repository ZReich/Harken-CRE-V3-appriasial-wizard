...
interface CompsLandMapListingProps {
...
const CompsLandMapListingIntegrated: React.FC<CompsLandMapListingProps> = (
  props
) => {
  ...
  const handleApiDataUpdate = useCallback(
    (
      ...
      console.log(
        '🌐🌐🌐 LAND MAP INTEGRATED - API data updated from map:',
        properties?.length,
        'properties, bounds:',
        bounds,
        'zoom:',
  ...
  const handleGoogleDataChange = useCallback((data: IComp[]) => {
    ...
    setFilteredData((currentFilteredData) => {
      if (currentFilteredData === null) {
        console.log(
          '🔄 LAND - Setting initial filteredData from listing component:',
          data.length,
          'items'
        );
        return data;
        ...
        console.log(
          '🔄 LAND - Keeping existing filteredData, not overriding with listing data'
        );
  ...
  const handleClustersApiUpdate = useCallback((clusters: any[]) => {
    console.log(
      '🌐🗺️ LAND PARENT - Clusters updated from listing API:',
      clusters.length,
      'clusters'
    );
