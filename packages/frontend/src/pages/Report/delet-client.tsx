...
const DeleteModalClient: React.FC<ConfirmationModalContentProps> = ({
  close,
  deleteId,
  setArrayAfterDelete,
}) => {
  const { mutate } = useMutate<ResponseType, any>({
    queryKey: 'client/delete/${deleteId}',
    endPoint: `client/delete/${deleteId}`,
  ...
  return (
    <DeleteModalContentAppraisal
      ...
      label={DeleteModalLabelEnum.DELETE_LIST}
      type="template"
    />
  );
};
