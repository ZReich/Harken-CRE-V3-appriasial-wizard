...
const EvaluationAerialInfoMap: React.FC<any> = () => {
  ...
  const mapZoomType = googleDta?.aerial_map_zoom || 14; // Default zoom level set to 14
  ...
  const mutation = useMutate<any, any>({
    queryKey: `evaluations/update-aerial-map/${id}`,
    endPoint: `evaluations/update-aerial-map/${id}`,
      ...
      <div className="w-1/3 py-9 px-5 absolute bg-white top-[330px] right-4">
        ...
        <div className="flex justify-center gap-3">
          <Button
            ...
            onClick={() => navigate(`/evaluation-property-boundaries?id=${id}`)}
          >
          ...
          <Button
            ...
            size="small"
            className="appraisal-previous-button text-xs !p-0 text-white font-medium h-[40px]"
            onClick={() => navigate(`/evaluation-area-info?id=${id}`)}
          >
            Skip
          </Button>
        </div>
...
export default EvaluationAerialInfoMap;
