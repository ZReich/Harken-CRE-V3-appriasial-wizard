...
const EvaluationAiCapCompsData = ({
  handleClose,
  onClose,
  open,
  compsData,
  ...
  useEffect(() => {
    ...
    if (checkType === 'leasesCheckbox' || approachType === 'leaseCheck') {
      formik.values.comps.forEach((comp: any, index: any) => {
        if (!comp.lease_rate_unit) {
          formik.setFieldValue(`comps[${index}].lease_rate_unit`, 'year');
        }
      });
    }
  }, [formik.values.comps]);
...
export default EvaluationAiCapCompsData;
