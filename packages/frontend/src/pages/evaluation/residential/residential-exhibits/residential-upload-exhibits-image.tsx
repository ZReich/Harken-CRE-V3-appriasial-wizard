...
const ResidentialUploadImageExhibits = ({
  refetch,
  setLoaderClassName,
  loaderClassName,
}: any) => {
  ...
  const { mutateAsync } = useMutate<any, FormDataObject>({
    queryKey: 'res-evaluations/upload-exhibits',
    endPoint: `res-evaluations/upload-exhibits/${id}`,
  ...
  if (loading) {
    return (
      <>
        <div
          className={`img-update-loader ${loaderClassName ? 'upload-exhibits-image' : ''}`}
        >
...
export default ResidentialUploadImageExhibits;
