...
import DeleteModalResidentialListing from './delete-confirmation-residential';
...
const ResidentialListingTable: React.FC<CompsListingMapProps> = ({
  ...
  // const [pdfHtml] = useState<string | null>(null);
  const [orderBy, setOrderBy] = useState<SortingEnum>(SortingEnum.DESC);
  const mutation = useMutate<any, any>({
    queryKey: `res-evaluations-list`,
    endPoint: 'res-evaluations/list',
  ...
  const editSetup = async (id: number, position: string) => {
    ...
    setDeleteId(id);
    try {
      if (position === 'residential-overview' || position === 'images') {
        position = '';
      }
      ...
      navigate(`/update-evaluation/residential-set-up?id=${id}`);
    }
  };
  ...
  // const { data: evaluation } = useGet<any>({
  //   queryKey: `evaluations/report-preview`,
  //   endPoint: `evaluations/report-preview/558`,
  // });
  const getScenarios = (evaluationId: number) => {
    const evaluation = data.find((item: any) => item.id === evaluationId);
    return evaluation?.res_evaluation_scenarios || [];
  };

  // const BASE_URL = import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_ROUTE || 'http://localhost:3001/api/v2/';
  // const getPdfEvaluation = async (id: number) => {
  //   try {
  //     const response = await axios.get(
  //       `${BASE_URL}/res-evaluations/report-preview/${id}`
  //     );
  ...
  //       newWindow.document.close(); // Finish writing and render content
  //     } else {
  //       alert('Please allow popups for this website to see the preview.');
  //     }
  //   } catch (error) {
                ...
                <TableCell>
                  ...
                  <span>
                    <Icons.PictureAsPdfIcon
                      className="text-red-500 cursor-pointer"
                      onClick={() =>
                        window.open(`/residential/report/${row?.id}`, '_blank')
                      }
                    />
                    {/* <Icons.PictureAsPdfIcon
                      className="text-red-500 cursor-pointer"
                      onClick={() => getPdfEvaluation(row.id)} // Replace with row?.id if mapping
                    />
...
export default ResidentialListingTable;
