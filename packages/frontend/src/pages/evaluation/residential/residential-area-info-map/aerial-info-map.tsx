...
import { ResidentialOverviewEnum } from '../residential-overview/residential-overview-enum';
...
const ResidentialEvaluationAerialInfoMap: React.FC<any> = () => {
  const navigate = useNavigate();
  const [map, setMap] = useState<google.maps.Map | null>(null);
  const [mapType, setMapType] = useState<any>(ResidentialOverviewEnum.ROAD_MAP);
  ...
  const mutation = useMutate<any, any>({
    queryKey: `res-evaluations/update-aerial-map/${id}`,
    endPoint: `res-evaluations/update-aerial-map/${id}`,
  ...
  const mapTypes =
    googleDta?.map_type || googleDta?.aerial_map_type || ResidentialOverviewEnum.ROAD_MAP;

  useEffect(() => {
    setMapType(
      mapTypes === ResidentialOverviewEnum.HYBRID
  ...
  const submitAerialMap = async () => {
    ...
    try {
      ...
      setLoader(false);
      toast(ResidentialOverviewEnum.AERIAL_MAP_SAVED_SUCCESSFULLY);
      navigate(`/residential/evaluation-area-info?id=${id}`);
    } catch (error) {
      console.error('Error updating aerial map:', error);
      toast.error(ResidentialOverviewEnum.ERROR_UPDATING_AERIAL_MAP);
    }
  };
  ...
  const handleMapTypeChange = (type: string) => {
    const newMapType =
      type === ResidentialOverviewEnum.HYBRID
        ? google.maps.MapTypeId.HYBRID
        : google.maps.MapTypeId.ROADMAP;
  ...
  return isLoaded ? (
    <ResidentialMenuOptions>
        ...
        onLoad={(map) => {
          ...
          map.setOptions({
            ...
            mapTypeControlOptions: {
              ...
              mapTypeIds: [ResidentialOverviewEnum.ROAD_MAP, ResidentialOverviewEnum.HYBRID],
            },
          });
          ...
          google.maps.event.addListener(map, 'maptypeid_changed', () => {
            ...
            handleMapTypeChange(
              currentMapType === ResidentialOverviewEnum.HYBRID ? ResidentialOverviewEnum.HYBRID : ResidentialOverviewEnum.ROAD_MAP
            );
          });

          google.maps.event.addListener(map, ResidentialOverviewEnum.DRAGEND, () => {
            handleDragEnd(map);
          });
        }}
      >
      ...
      <div className="w-1/3 py-9 px-5 absolute bg-white top-[330px] right-4">
        <p className="font-bold pb-2 text-center">{ResidentialOverviewEnum.AERIAL_MAP_HEADER}</p>
        <p className="text-center text-xs pb-4">
          {ResidentialOverviewEnum.AERIAL_MAP_TEXT}
        </p>
...
export default ResidentialEvaluationAerialInfoMap;
