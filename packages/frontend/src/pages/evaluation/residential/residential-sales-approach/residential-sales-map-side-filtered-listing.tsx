...
const ResidentialSalesMapSideFilteredListing: React.FC<
  CompsListingMapProps
> = ({
  ...
  console.log('🏠🎬 SideListingMap - Component rendered with props:', {
  ...
  console.log(
    '🏠 SideListingMap - externalData type:',
    typeof externalData,
    setDeleteId
  );
  console.log(
  ...
  const valuess = location.state?.values;
  ...
  const calculateAdjustmentValue = (
    ...
    const expenseConfig = (valuess as any).operatingExpenses.find(
    ...
    if (['time', 'location', 'condition'].includes(expense.adj_key)) {
      ...
      const finishedConfig = (values as any).operatingExpenses.find(
        (exp: any) => exp.adj_key === 'basement_finished'
      );
      const unfinishedConfig = (values as any).operatingExpenses.find(
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const appraisalData = evaluationResponse?.data?.data?.data;
      ...
      if (response?.data) {
        ...
        // Extract original values from valuess.operatingExpenses
        const originalOperatingExpenses = valuess.operatingExpenses || [];

        const updatedComps = comps.map(({ id, ...restComp }: any) => {
          // Create a map of original values by adj_key for quick lookup
          const originalValuesMap: { [key: string]: any } = {};
          originalOperatingExpenses.forEach(
            (exp: { adj_key: string; adj_value: any }) => {
              originalValuesMap[exp.adj_key] = exp.adj_value;
            }
          );

          const calculatedExpenses = [
            // Use original values from valuess.operatingExpenses and calculate adjustments
            ...originalOperatingExpenses.map((exp: any) => {
              // Calculate the adjustment value for this expense
              const calculatedValue = calculateAdjustmentValue(
                exp,
                restComp,
                appraisalData
              );
          ...
          return {
            ...
            // Add the original operating expenses as well
...
export default ResidentialSalesMapSideFilteredListing;
