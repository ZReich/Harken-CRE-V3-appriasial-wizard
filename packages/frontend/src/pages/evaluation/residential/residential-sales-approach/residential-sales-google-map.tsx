...
// Default center and zoom for initial map load (US-centered for wide coverage)
const defaultCenter = {
  lat: 39.5, // Better center for continental US
  lng: -98.35, // Better longitude for continental US center
};
const defaultZoom = 5; // Better zoom level to show US clusters properly
...
const GoogleMapLocationComps: React.FC<SendTypeItem> = ({
  ...
  const viewListingItem = (id: number) => {
    ...
    navigate(`/residential/sales-comps-view/${id}/${check}`, {
      state: {
  ...
  const callBothAPIs = useCallback(
    async (map: google.maps.Map, useWorldBounds = false) => {
      ...
      if (useWorldBounds) {
        ...
        east = 180;
        west = -180;
        zoom = calculatedZoom || 5; // Use calculated zoom or fallback
      } else {
      ...
      try {
        ...
        const getUrlBasedParams = () => {
          ...
          // Return filters object structure - only include parameters accepted by residential sales API
          return {
  ...
  const handleMapLoad = useCallback(
    (map: google.maps.Map) => {
      ...
      // Add zoom change listener with 2-second delay
  ...
  const createClusterTooltip = (cluster: any): string => {
    const avgPrice = formatPrice(cluster.avgPrice);
    const avgSize = formatPrice(cluster.avgSize);
    const avgPricePerSf = formatPrice(cluster.avgPricePerSf);
          ...
          if ((cluster as any).bounds) {
            ...
            google.maps.event.addListenerOnce(map, 'bounds_changed', () => {
              ...
              if (currentZoom && currentZoom > 16) {
                map.setZoom(16); // Don't zoom in too close
              }
            ...
            >
              ...
              <Box className="text-sm flex-1 py-3">
                ...
                >
                  ...
                  <Box className="">
                    <a href={`/residential/sales-comps-view/${elem.id}`}>
                      <CommonButton
                        variant="contained"
                        className="text-xs"
                        color="primary"
    ...
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {loading && (
        ...
        >
          ...
          <span style={{ fontSize: '12px' }}>Loading clusters...</span>
        </div>
      )}

      {/* API call notification */}

      {/* Interaction indicator - centered loader with map overlay */}
      {isInteracting && !loading && (
        <>
          {/* Semi-transparent overlay to reduce map opacity */}
          <div
            style={{
              position: 'absolute',
              top: 0,
      ...
      {/* Loading indicator - only show when actually loading data */}
      {loading && (
        <div
          style={{
            position: 'absolute',
        ...
        >
          <CircularProgress size={20} />
          <span style={{ fontSize: '14px' }}>Loading properties...</span>
        </div>
      )}
