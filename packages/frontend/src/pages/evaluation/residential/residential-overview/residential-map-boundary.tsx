...
const ResidentialEvalouationMapBoundary = () => {
  ...
  const mutation = useMutate<any, any>({
    queryKey: 'logins',
    endPoint: `res-evaluations/update-map-boundary/${id}`,
    requestType: RequestType.PATCH,
  });
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (polygonData.length === 0) {
      navigate(`/residential/evaluation-aerialmap?id=${id}`);
      return;
    }
      ...
      <ResidentialMenuOptions>
        ...
        <div className="w-1/3 py-9 px-5 absolute bg-white top-[330px] right-4">
          <p className="font-bold pb-2 text-center">
            {ResidentialOverviewEnum.MAP_BOUNDARY_HEADER}
          </p>
          <p className="text-center text-xs pb-4">
            {ResidentialOverviewEnum.MAP_BOUNDARY_TEXT}
          </p>
...
export default ResidentialEvalouationMapBoundary;
