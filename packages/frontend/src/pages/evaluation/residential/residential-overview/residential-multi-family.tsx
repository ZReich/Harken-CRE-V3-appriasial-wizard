...
export const ResidentialMultiFamily = () => {
  ...
  const fetchComposData = async () => {
    try {
      const response = await axios.get(
        `evaluations/get-rent-roll?evaluationId=${id}&evaluationApproachId=${appraisalId}`
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (STATE_ID) {
        const matchedItem = data?.data?.data?.evaluation_id.find(
  ...
  const handleSubmit = () => {
    ...
    const params = {
      evaluation_id: id,
      evaluation_approach_id: appraisalId,
      type,
      rent_rolls: details,
    };
      ...
      .then((response) => {
        ...
        if (nextApproach) {
          switch (nextApproach?.type) {
            case 'rent_roll':
              navigate(
                `/evaluation/rent-roll?id=${id}&appraisalId=${nextApproach.id}`
              );
              setRollId('');
            ...
            case 'sale':
              navigate(
                `/evaluation/sales-approach?id=${id}&salesId=${nextApproach.id}`
              );
              break;
            case 'cost':
              navigate(
                `/evaluation/cost-approach?id=${id}&costId=${nextApproach.id}`
              );
              break;
            case 'lease':
              navigate(
                `/evaluation/lease-approach?id=${id}&leaseId=${nextApproach.id}`
              );
              break;
            default:
              navigate(`/evaluation/appraisal-exhibits?id=${id}`);
          }
        } else {
          navigate(`/evaluation/appraisal-exhibits?id=${id}`);
        }
  ...
  const handleUpdateSubmit = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const appraisalApproaches = data?.data?.data?.evaluation_id || [];
              ...
              onClick={() => {
                if (filtereRentRollData && filtereRentRollData.length > 1) {
                  ...
                  if (rentRollIndex > 0) {
                    ...
                    navigate(
                      `/evaluation/rent-roll?id=${id}&appraisalId=${previousRentRollId}`
                    );
                    return;
                  }
                }
                ...
                if (filtereIncomeData?.length > 0) {
                  ...
                  if (lastIncomeId) {
                    navigate(
                      `/evaluation/income-approch?id=${id}&IncomeId=${lastIncomeId}`
                    );
                    return;
                  }
                }
