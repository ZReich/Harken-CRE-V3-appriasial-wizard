...
export const UpdatedEvaluationSetup = () => {
  ...
  const [clientId, setClientID] = useState<any>(localStorage.getItem('client_id'));
  const [setDataRequited] = React.useState(false);
  ...
  useEffect(() => {
    const hasSalesApproach = setData?.scenarios?.some((item: any) => item.has_sales_approach === 1);
    if (hasSalesApproach) {
  ...
  const transformedScenarios =
    setData?.scenarios?.map((scenario: any) => ({
  ...
  const { mutate: mutateUpdate } = useMutate<any, any>({
    queryKey: 'evaluations/update-setup',
    endPoint: `evaluations/update-setup/${id}`,
  ...
  const validateUpdateEvaluationFields = (
    ...
    if (invalidApproach) {
      showErrorToast(
        `Please select at least one approach for "${invalidApproach.name}" scenario.`
      );
      return false;
    }
                                      ...
                                      <FormControlLabel
                                        control={
                                          <Checkbox
                                            checked={
                                              values.scenarios[index]
                                                .has_multi_family_approach === 1
                                            }
                                            onChange={(e) =>
                                              setFieldValue(
                                                `scenarios[${index}].has_multi_family_approach`,
                                                e.target.checked ? 1 : 0
                                              )
                                            }
                                          />
                                      ...
                                      <FormControlLabel
                                        control={
                                          <Checkbox
                                            ...
                                            onChange={(e) =>
                                              setFieldValue(
                                                `scenarios[${index}].has_cap_approach`,
                                                e.target.checked ? 1 : 0
                                              )
                                            }
                                          />
                                    ...
                                    <FormControlLabel
                                      control={
                                        <Checkbox
                                          checked={
                                            values.scenarios[index]
                                              .has_lease_approach === 1
                                          }
                                          onChange={(e) =>
                                            setFieldValue(
                                              `scenarios[${index}].has_lease_approach`,
                                              e.target.checked ? 1 : 0
                                            )
                                          }
                                        />
