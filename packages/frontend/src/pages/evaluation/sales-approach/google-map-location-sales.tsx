...
const GoogleMapLocationSales: React.FC<SendTypeItem> = ({
  GoogleData,
  ...
  const viewListingItem = (id: number) => {
    ...
    if (property_type === 'salesCheckbox' || !property_type) {
      // if "salesCheckbox" OR not set in localStorage → default to "sales"
      check = 'sales';
    } else {
      check = 'lease';
    }
  ...
  const callBothAPIs = useCallback(
    ...
    [onApiDataUpdate, onMapClustersChange, filterParams]
  );
  ...
  const handleClusterClick = async (cluster: ClusterData) => {
    if (map) {
      if (cluster.count === 1) {
        const property = GoogleData?.find((prop) => {
        ...
        // Zoom to maximum level (20) for single property clusters
        map.setZoom(20);
            ...
            >
              ...
              <Box className="text-sm flex-1 py-3">
                ...
                >
                  ...
                  <Box className="">
                    <a href={`/evaluation/sales-comps-view/${elem.id}`}>
                      <CommonButton
                        variant="contained"
                        className="text-xs"
                        color="primary"
      ...
      >
        ...
        <button
          onClick={() => {
            if (map) {
              const currentZoom = map.getZoom() || 0;
              map.setZoom(Math.max(currentZoom - 2, 4));
            }
          }}
          ...
          style={{
            width: '40px',
            height: '40px',
            backgroundColor: currentZoom <= 4 ? '#f5f5f5' : 'white',
            border: 'none',
            fontSize: '28px',
            fontWeight: 'medium',
      ...
      >
        ...
        {currentZoom < 15 &&
          clusters.map((cluster, index) => {
            ...
            // For single property clusters, find the property to show hover tooltip
            const singleProperty =
              cluster.count === 1
                ? GoogleData?.find((prop) => {
                    const lat = parseFloat(prop.map_pin_lat || prop.latitude);
                    const lng = parseFloat(prop.map_pin_lng || prop.longitude);
                    return (
                      Math.abs(lat - cluster.lat) < 0.001 &&
            ...
            return (
              <Marker
                ...
                onMouseOver={() => {
                  ...
                  // Show hover tooltip for single property clusters
                  if (cluster.count === 1 && singleProperty) {
                    if (hoverTimeout) {
                ...
                onMouseOut={() => {
                  ...
                  // Clear hover tooltip for single property clusters
                  if (cluster.count === 1 && hoverTimeout) {
                    clearTimeout(hoverTimeout);
        ...
        {currentZoom > 14 &&
          ...
          clusters.map((cluster, index) => {
            const clusterId =
              (cluster as any).id ||
              `property-marker-${index}-${cluster.lat}-${cluster.lng}`;

            // Find the property for this cluster location
            const property = GoogleData?.find((prop) => {
              const lat = parseFloat(prop.map_pin_lat || prop.latitude);
              const lng = parseFloat(prop.map_pin_lng || prop.longitude);
              return (
            ...
            return (
              <Marker
                ...
                title={
                  property
                    ? `${property.street_address} - ${formattedPrice}`
                    : formattedPrice
                }
                onClick={() => {
                  if (property) {
        ...
        {currentZoom >= 15 &&
          ...
          (() => {
            ...
            const lng = parseFloat(property.map_pin_lng || property.longitude);
        ...
        {/* Simple hover tooltip for price display - only for single cluster hover */}
        {hoverTooltip && (
          <InfoWindow
            key={`hover-tooltip-${hoverTooltip.property.id}`}
          ...
          >
            ...
            >
              ...
              >
                {property_type === 'salesCheckbox' || !property_type
                  ? hoverTooltip.property.sale_price
                    ? `$${hoverTooltip.property.sale_price.toLocaleString()}`
                    : 'N/A'
                  : hoverTooltip.property.lease_rate
                    ? `$${hoverTooltip.property.lease_rate.toLocaleString()}`
                    : 'N/A'}
              </div>
            </div>
          </InfoWindow>
...
export default GoogleMapLocationSales;
