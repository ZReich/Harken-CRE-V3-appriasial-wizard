...
const EvaluationSalesMapSideFilteredListing: React.FC<CompsListingMapProps> = ({
  ...
  console.log(setApiData, setApiPaginationMeta, setIsLoadingApi);
  ...
  // Function to get filter parameters from localStorage and props

  // API call function for cluster-details
  // const callClusterDetailsAPI = async () => {
  //   setIsLoadingApi(true);
  //   try {
  //     // Get filter parameters
  //     const filters = getFilterParams();

  //     // Parameters for cluster-details API with filters object structure
  //     const params = {
  //       bounds: {
  //         north: 85,
  //         south: -85,
  ...
  //       page: page,
  //       limit: 10,
  //       filters: filters, // All filter parameters in filters object
  //     };

  //     console.log(
  //       '🌐 DEBUG: Listing - Full API params:',
  //       JSON.stringify(params, null, 2)
  //     );
  //     console.log(
  //       '🌐 DEBUG: Listing - City in API params:',
  //       params.filters.city
  //     );
  //     console.log(
  //       '🌐 DEBUG: Listing - City array length:',
  //       params.filters.city?.length
  //     );

  //     // Call cluster-details API
  //     const response = await APIClient.getInstance().post<any, typeof params>(
  //       'comps/map-cluster-details',
  //       params
  //     );

  //     console.log('🌐 Listing: Cluster-details API response:', response);

  //     // Process cluster-details API response
  //     const properties = response.data?.data?.properties || [];
  //     const apiMetadata = response.data?.data || {};

  //     console.log('🌐 Listing: Cluster-details properties:', properties.length);

  //     setApiData(properties);
  //     setApiPaginationMeta({
  //       totalRecord: apiMetadata.totalRecord,
  //       totalPages: apiMetadata.totalPages,
  //       limit: apiMetadata.limit,
  //       page: apiMetadata.page,
  //     });
  //   } catch (error) {
  //     console.error('🌐 Error calling cluster-details API in listing:', error);
  //     setApiData([]);
  //     setApiPaginationMeta({});
  //   } finally {
  ...
  // Keep this effect for backward compatibility with other props
  useEffect(() => {
  ...
  const viewListingItem = (id: number) => {
    ...
    navigate(`/evaluation/sales-comps-view/${id}/${check}`, {
      state: {
  ...
  // Use mapData from map component (prioritized since it reflects current viewport)
  // Fallback to apiData for backward compatibility
  const data = mapData && mapData.length > 0 ? mapData : apiData || [];

  // Get pagination data - prioritize map pagination metadata (from map component)
  // Fallback to API pagination metadata for backward compatibility
                          ...
                          <Box className="text-sm flex-1 py-3 min-w-52">
                            ...
                            >
                              ...
                              <Box>
                                <Link
                                  to={{
                                    pathname: `/evaluation/sales-comps-view/${ele.id}`,
                                  }}
                                  state={{
                                    id: 880,
                                    approachId: 331,
...
export default EvaluationSalesMapSideFilteredListing;
