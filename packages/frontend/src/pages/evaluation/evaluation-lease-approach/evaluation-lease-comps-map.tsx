...
const EvaluationCompsGoogleMapAreaInfoLease: React.FC<any> = () => {
  ...
  const { data: leaseApproch, refetch: refetchSale } = useGet<any>({
    queryKey: `evaluations/get-lease-approach?evaluationId=${id}&evaluationScenarioId=${leaseMapId}`,
    endPoint: `evaluations/get-lease-approach?evaluationId=${id}&evaluationScenarioId=${leaseMapId}`,
  ...
  console.log(leaseId, 'leaseIdleaseId');
  const latLngArray =
    salesMap?.map(
  ...
  const mutation = useMutate<any, any>({
    queryKey: `evaluations/save-lease-area-map/${leaseId}`,
    endPoint: `evaluations/save-lease-area-map/${leaseId}`,
    requestType: RequestType.PATCH,
  });
  useEffect(() => {
    if (lat && long) {
    ...
    try {
      ...
      if (hasLeaseType && filtereleasedData.length > 1) {
        ...
        if (leaseIndex < filtereleasedData.length - 1) {
          ...
          navigate(
            `/evaluation/lease-approach?id=${id}&leaseId=${saleIdRedirectIndex}`
          );
          return;
        }
      }
      ...
      <div className="w-1/3 py-9 px-5 absolute bg-white top-[330px] right-4">
        ...
        <div className="flex justify-center gap-3">
          <Button
            ...
            onClick={() => {
              navigate(
                `/evaluation/lease-approach?id=${id}&leaseId=${leaseMapId}`
              );
            }}
          >
...
export default EvaluationCompsGoogleMapAreaInfoLease;
