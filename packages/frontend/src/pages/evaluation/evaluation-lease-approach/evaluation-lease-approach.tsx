...
export const calculateCompData = ({
  ...
  if (appraisalData?.comp_adjustment_mode === 'Dollar') {
    if (appraisalData?.comp_type === 'land_only') {
      if (appraisalData?.analysis_type === '$/Acre') {
        updatedAdjustedPsf =
          total + parseFloat((price_square_foot / 43560).toFixed(2));
      } else {
    ...
    if (appraisalData?.comp_type === 'land_only') {
      if (appraisalData?.analysis_type === '$/Acre') {
        updatedAdjustedPsf =
          (total / 100) * parseFloat((price_square_foot / 43560).toFixed(2)) +
          parseFloat((price_square_foot / 43560).toFixed(2));
      } else {
        updatedAdjustedPsf =
...
const EvaluationLeaseApproach: React.FC = () => {
  ...
  useEffect(() => {
    if (areaInfoData?.data?.data?.scenarios && !areaInfoData.isStale) {
      ...
      setHasCapRateType(capApproaches.length > 0);
  ...
  const handleSubmitWithUpdatedComps = async (updatedComps: any) => {
    try {
      ...
      if (updatedMinAveragedAdjustedPsf === Infinity)
        updatedMinAveragedAdjustedPsf = 0;
      if (updatedMaxAveragedAdjustedPsf === -Infinity)
        updatedMaxAveragedAdjustedPsf = 0;
