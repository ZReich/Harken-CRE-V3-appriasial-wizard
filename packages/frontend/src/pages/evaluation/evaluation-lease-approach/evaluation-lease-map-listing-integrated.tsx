...
import GoogleMapLocationLease from './google-map-location-lease';
...
> = (props) => {
  ...
  const handleApiDataUpdate = useCallback(
    (
      ...
      console.log(
        '🌐🌐🌐 LEASE MAP INTEGRATED - API data updated from map:',
        properties?.length,
        'properties, bounds:',
        bounds,
        'zoom:',
  ...
  const passGoogleData = useCallback((data: IComp[]) => {
    console.log(
      '🔄 LEASE - Data from listing component:',
      data.length,
      'items'
    );
    setMapData(data);
  ...
  const filterParamsWithLocalStorage = React.useMemo(() => {
    console.log(
      '🔄 LEASE INTEGRATED: Re-computing filterParamsWithLocalStorage due to prop/localStorage changes'
    );
    return {
      comparison_basis: (props.sidebarFilters as any)?.comparison_baisi || 'SF',
      comparisonBasis: props.comparisonBasisView || 'SF',
  ...
  useEffect(() => {
    const handleStorageChange = () => {
      console.log(
        '🔄 LEASE INTEGRATED: localStorage changed, triggering filter update'
      );
