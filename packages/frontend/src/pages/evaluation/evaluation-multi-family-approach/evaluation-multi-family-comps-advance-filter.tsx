...
const EvaluationMultiFamilyCompsAdvanceFilter: React.FC<MapFilterProps> = ({
  isOpen,
  setIsOpen,
  onApplyFilter,
  passDataCheckType,
  ...
  // Initialize selectedCities from localStorage on component mount
  useEffect(() => {
    const storedCities = localStorage.getItem('selectedCities');
    console.log('🔍 FILTER INIT: Loading stored cities:', storedCities);
    ...
    if (storedCities) {
      try {
        ...
        if (Array.isArray(parsedCities) && parsedCities.length > 0) {
          ...
          console.log(
            '🏙️ FILTER INIT: Loaded cities from localStorage:',
            parsedCities
          );
        }
      } catch (e) {
  ...
  useEffect(() => {
    // Only clear selectedCities if state changes and there are no stored cities
    const storedCities = localStorage.getItem('selectedCities');
    if (!storedCities) {
      setSelectedCities([]);
    }
  ...
  const handleCityChange = (selectedCities: City[]) => {
    ...
    console.log('🏙️ FILTER: Selected cities:', cityNames);
    console.log(
      '🏙️ FILTER: Storing in localStorage:',
  ...
  const handleSubmit = async (value: FormData) => {
    ...
    // Get cities from localStorage and ensure it's properly parsed
    let selectedCityArray = [];
    const storedCities = localStorage.getItem('selectedCities');

    console.log('🔍 FILTER SUBMIT: Raw stored cities:', storedCities);
    console.log(
      '🔍 FILTER SUBMIT: Current selectedCities state:',
      selectedCities
    );
    ...
    if (storedCities) {
      try {
        const parsedCities = JSON.parse(storedCities);
        selectedCityArray = Array.isArray(parsedCities) ? parsedCities : [];
        console.log(
          '🏙️ FILTER SUBMIT: Parsed cities array:',
          selectedCityArray
        );
      } catch (e) {
        console.warn('🚫 FILTER SUBMIT: Failed to parse stored cities:', e);
        selectedCityArray = [];
      }
    } else if (selectedCities && selectedCities.length > 0) {
      // Fallback to state if localStorage is empty
      selectedCityArray = selectedCities;
      console.log(
        '🏙️ FILTER SUBMIT: Using cities from state:',
        selectedCityArray
      );
    }
    ...
    const params: Omit<FilterComp, 'search'> = {
      ...
      city: selectedCityArray || [], // Use the properly parsed city array
      state: stateStorage || '',
      comp_type: undefined,
      building_sf_min:
    ...
    console.log('🔍 FILTER SUBMIT: Final params object:', params);
    console.log('🏙️ FILTER SUBMIT: City field in params:', params.city);
    console.log('🏙️ FILTER SUBMIT: City array length:', params.city?.length);
                      ...
                      <div className="grid grid-cols-3 gap-5">
                        ...
                        <div className="w-full">
                          <AdvanceFilterMultipleSelectCheckmarks
                            ...
                            usaCity={usaCity}
                            value={selectedCities.map((city) => ({ city }))} // Convert city names to City objects
                            label="Commercial"
...
export default EvaluationMultiFamilyCompsAdvanceFilter;
