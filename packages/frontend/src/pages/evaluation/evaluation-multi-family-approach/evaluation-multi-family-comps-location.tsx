...
const EvaluationGoogleMapAreaInfoMultiFamily: React.FC<any> = () => {
  ...
  const costMapId = searchParams.get('evaluationId');
  ...
  const mutation = useMutate<any, any>({
    queryKey: 'costMap',
    endPoint: `evaluations/save-multi-family-area-map/${salesId}`,
    requestType: RequestType.PATCH,
  });
  ...
  const submitAerialMap = async () => {
    ...
    try {
      ...
      toast('Multi Family comps map saved successfully!');
      const nextIncomeScenario = data?.data?.data?.scenarios.find(
        (scenario: any) =>
      ...
      if (nextIncomeScenario) {
        navigate(
          `/evaluation/income-approch?id=${id}&IncomeId=${nextIncomeScenario.id}`
        );
        return;
      }
      const nextCapScenario = data?.data?.data?.scenarios.find(
        (scenario: any) =>
          scenario.id !== STATE_ID &&
          scenario.has_cap_approach === 1 &&
          scenario.id > Number(STATE_ID)
      );

      if (nextCapScenario) {
        navigate(
          `/evaluation/cap-approach?id=${id}&capId=${nextCapScenario.id}`
        );
        return;
      }

      // Get scenarios from the main data
      if (hasMultiFamilyType && filtereMultiFamilydData.length > 1) {
        const multiFamilyApproachId = searchParams.get('evaluationId');

        const multiFamilyIndex = filtereMultiFamilydData.findIndex(
          (element) => element.id == multiFamilyApproachId
        );

        if (multiFamilyIndex < filtereMultiFamilydData.length - 1) {
          const multiFamilyIdRedirectIndex =
            filtereMultiFamilydData[multiFamilyIndex + 1].id;
          navigate(
            `/evaluation/rent-roll?id=${id}&evaluationId=${multiFamilyIdRedirectIndex}`
          );
          return;
        }
      }
  ...
  return isLoaded ? (
    <EvaluationMenuOptions>
      <div className="px-5 py-5 flex justify-between items-center">
        <h1 className="text-2xl font-bold uppercase">Multi Family Comp Map</h1>
      ...
      <div className="w-1/3 py-9 px-5 absolute bg-white top-[330px] right-4">
        <p className="font-bold pb-2 text-center">
          Set the Multi Family Comp map view
        </p>
        ...
        <div className="flex justify-center gap-3">
          <Button
            ...
            onClick={() => {
              navigate(
                `/evaluation/multi-family-approach?id=${id}&evaluationId=${costMapId}`
              );
            }}
          >
...
export default EvaluationGoogleMapAreaInfoMultiFamily;
