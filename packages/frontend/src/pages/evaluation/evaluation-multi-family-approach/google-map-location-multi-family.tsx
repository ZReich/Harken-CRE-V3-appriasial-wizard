...
const GoogleMapLocationMultiFamily: React.FC<SendTypeItem> = ({
  GoogleData,
  ...
  const viewListingItem = (id: number) => {
    ...
    navigate(`/evaluation/multi-family-comps-view/${id}/${check}`, {
      state: {
        ...
        {hoveredInfoWindow && false && (
          ...
          >
            ...
            >
              ...
              <Box className="flex-1 py-3">
                <Box className="flex mb-1.5">
                  <Tooltip title={hoveredInfoWindow?.property.street_address}>
                    <Box
                      className="text-[#0DA1C7] text-base font-semibold capitalize leading-6 MuiBox-root css-0"
                      style={{ paddingRight: '0.3rem' }}
                    >
                ...
                {localStorage.getItem('activeType') === 'building_with_land' ? (
                  <Box className="flex gap-1 text-sm flex-wrap">
                    ...
                    <p className="font-normal text-gray-900">
                      {(hoveredInfoWindow?.property as any)?.year_built ===
                        null ||
                      (hoveredInfoWindow?.property as any)?.year_built === ''
                        ? 'N/A'
                        : (hoveredInfoWindow?.property as any)?.year_built}
              ...
              <Box className="text-sm flex-1 py-3">
                {localStorage.getItem('activeType') !== 'land_only' ? (
                  <Box className="flex gap-1 flex-wrap">
                    ...
                    <Tooltip
                      title={
                        hoveredInfoWindow?.property?.building_size === null ||
                        hoveredInfoWindow?.property?.building_size === 0
                          ? 'N/A'
                ...
                {localStorage.getItem('activeType') === 'building_with_land' ? (
                  <Box className="flex gap-1 flex-wrap">
                    ...
                    <Tooltip
                      title={
                        (hoveredInfoWindow?.property as any).land_dimension ===
                        'ACRE'
                          ? hoveredInfoWindow?.property?.land_size === null ||
                            hoveredInfoWindow?.property?.land_size === 0
                            ? 'N/A'
                            : (
                                (hoveredInfoWindow?.property?.land_size ?? 0) *
                                43560
                  ...
                  <Box className="flex gap-1 flex-wrap">
                    ...
                    <Tooltip
                      title={
                        selectedToggleButton === 'SF'
                          ? (hoveredInfoWindow?.property as any)
                              .land_dimension === 'ACRE'
                            ? hoveredInfoWindow?.property?.land_size === null ||
                              hoveredInfoWindow?.property?.land_size === 0
                              ? 'N/A'
                ...
                {property_type === 'salesCheckbox' || !property_type ? (
                  <Box className="flex gap-1 flex-wrap">
                    <p className="text-sm font-normal text-gray-900">
                      {hoveredInfoWindow?.property?.sale_status === 'Pending'
                        ? 'Sale Status :'
                        : 'Date of Sale :'}
                    </p>
                    <Tooltip
                      title={
                        hoveredInfoWindow?.property?.sale_status === 'Pending'
                          ? hoveredInfoWindow?.property?.sale_status
...
export default GoogleMapLocationMultiFamily;
