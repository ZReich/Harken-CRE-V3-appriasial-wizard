...
const EvaluationGoogleMapAreaInfoCost: React.FC<any> = () => {
  ...
  const { data: costApproch, refetch: refetchCost } = useGet<any>({
    queryKey: `evaluations/get-cost-approach?evaluationId=${id}&evaluationScenarioId=${costMapId}`,
    endPoint: `evaluations/get-cost-approach?evaluationId=${id}&evaluationScenarioId=${costMapId}`,
  ...
  const mutation = useMutate<any, any>({
    queryKey: 'costMap',
    endPoint: `evaluations/save-cost-area-map/${salesId}`,
    requestType: RequestType.PATCH,
  });
...
export default React.memo(EvaluationGoogleMapAreaInfoCost);
