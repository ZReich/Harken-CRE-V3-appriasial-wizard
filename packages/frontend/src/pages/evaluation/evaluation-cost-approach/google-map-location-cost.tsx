...
const defaultZoom = 5; // Optimal zoom level to show continental US

const GoogleMapLocationCost: React.FC<SendTypeItem> = ({
  GoogleData,
  clustersData,
  ...
  const callBothAPIs = useCallback(
    async (map: google.maps.Map, useWorldBounds = false) => {
      ...
      if (useWorldBounds) {
        // Use world bounds for initial load but maintain US center view
        north = 85;
        south = -85;
        east = 180;
        west = -180;
        zoom = 5; // Always use US zoom level for initial load

        // Always ensure map is centered on United States
        if (map) {
          map.setCenter(defaultCenter); // US center
          map.setZoom(defaultZoom); // US-appropriate zoom
        }
      } else {
      ...
      try {
        ...
        if (clusters && Array.isArray(clusters) && clusters.length > 0) {
          ...
          // Maintain US center view - do not auto-calculate viewport from clusters
          if (useWorldBounds && !calculatedZoom) {
            // Set fixed US center and zoom instead of calculating from clusters
            setCalculatedZoom(defaultZoom);
            // setCalculatedCenter(defaultCenter);

            // Keep map centered on US
            if (map) {
  ...
  const handleMapLoad = useCallback(
    ...
    [handleZoomChange, callBothAPIs, clustersData, debouncedApiCall]
  );
            ...
            >
              ...
              <Box className="text-sm flex-1 py-3">
                      ...
                      selectedToggleButton === 'SF' // Check if the selected toggle is "SF"
                        ...
                        : selectedToggleButton === 'AC' // Check if the selected toggle is "ACRE"
                          ? (elem as any).land_dimension === 'SF' // If the land dimension is in "SF"
                            ...
                            : elem?.land_size === null || elem?.land_size === 0 // If already in ACRE, just check for null or empty value
                              ? 'N/A'
                ...
                >
                  ...
                  <Box className="">
                    <a href={`/evaluation/cost-comps-view/${elem.id}`}>
                      <CommonButton
                        variant="contained"
                        className="text-xs"
                        color="primary"
  ...
  return isLoaded ? (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <GoogleMap
        ...
        center={defaultCenter} // Always use US center
        zoom={defaultZoom} // Always use US zoom
        options={{
                ...
                <Box className="text-sm flex-1 py-3">
                    ...
                    <Tooltip
                      title={
                        selectedToggleButton === 'SF'
                          ? hoveredInfoWindow?.property?.land_dimension ===
                            'ACRE'
                            ? !hoveredInfoWindow?.property?.land_size
                              ? 'N/A'
                              : (
                                  hoveredInfoWindow?.property?.land_size ??
                                  0 * 43560
                            ...
                            : !hoveredInfoWindow?.property?.land_size
                              ? 'N/A'
                              : hoveredInfoWindow?.property?.land_size?.toLocaleString() +
                                ' SF'
                          : selectedToggleButton === 'AC'
                            ? hoveredInfoWindow?.property?.land_dimension ===
                              ...
                              : !hoveredInfoWindow?.property?.land_size
                                ? 'N/A'
                                : hoveredInfoWindow?.property?.land_size?.toFixed(
                                    3
                                  ) + ' AC'
                            : 'N/A'
                      }
                  ...
                  {property_type === 'salesCheckbox' || !property_type ? (
                    <Box className="flex gap-1 flex-wrap">
                      ...
                      <p className="text-[13px] font-normal text-gray-900">
                        {formatDateToMMDDYYYY(
                          hoveredInfoWindow?.property?.date_sold ?? ''
                        )}
                      </p>
                    </Box>
                  ) : (
                    <Box className="flex gap-1 flex-wrap">
                      ...
                      <p className="font-normal text-gray-900">
                        {hoveredInfoWindow?.property?.lease_status ===
                          'select' ||
                        hoveredInfoWindow?.property?.lease_status === ''
                          ? 'N/A'
                          : hoveredInfoWindow?.property?.lease_status ===
                  ...
                  >
                    ...
                    <Box className="">
                      <a
                        href={`/comps-view/${hoveredInfoWindow?.property?.id}`}
                      >
                        <CommonButton
                          variant="contained"
                          className="text-xs"
                    ...
                    {!hideEditButton && (
                      <Box className="ml-1 mr-2">
                        <a
                          href={`/update-comps/${hoveredInfoWindow?.property?.id}`}
                          onClick={(e) => {
                            e.preventDefault();
                            if (hoveredInfoWindow?.property?.id) {
                              updateListingItem(hoveredInfoWindow.property.id);
                            }
                          }}
                        >
                          <CommonButton
...
export default React.memo(GoogleMapLocationCost);
