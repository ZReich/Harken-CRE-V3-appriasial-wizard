...
// import { sizeTypeOptions } from '@/pages/comps/create-comp/SelectOption';
...
import { EvaluationLandOnlyProperty } from './evaluation-land-only-property';
import EvaluationIncludedUtilities from './evaluation-included-utilities';
import { useGet } from '@/hook/useGet';
import { useSearchParams } from 'react-router-dom';
// import useGlobalCodeOptions from '../globalCodes/global-codes-option';
export const EvlaluationSpecification = ({
  getCompType,
  updateData,
  passRequired,
  showIncludedUtilities,
  ...
  const { data } = useGet<any>({
    queryKey: 'evaluations/check-linked-comps',
    endPoint: `evaluations/check-linked-comps/${id}`,
    config: {},
  });
  console.log(data);
  // const checkLinkMessage = data?.data?.message;
  console.log(hasMultiFamilyApproach, 'checkLinkMessage');

  console.log(showIncludedUtilities, 'showIncludedUtilities');
  // const [searchParams] = useSearchParams();
  // const id = searchParams.get('id');
  ...
  // Set comparison_basis to 'Unit' when hasMultiFamilyApproach is 1
  useEffect(() => {
    if (hasMultiFamilyApproach === 1 && values.comparison_basis !== 'Unit') {
      setFieldValue(SpecificationEnum.COMPARISION_BASIS_NAME, 'Unit');
    }
  }, [hasMultiFamilyApproach, setFieldValue]);
      ...
      <div>
          ...
          {localStorage.getItem('activeType') === 'building_with_land' ? (
            <Grid item xs={6} className="pt-2">
              <SelectTextField
                ...
                disabled={hasMultiFamilyApproach === 1}
                value={
        ...
        <Grid container spacing={3} className="mt-4 items-end">
          {localStorage.getItem('activeType') === 'land_only' ? (
            <Grid
              className="pt-2.5 selectFixedHeight"
              item
          ...
          <Grid item xs={2.5} className="pt-2.5 ">
            <StyledField
              label={
                <span className="text-customGray">
          ...
          <Grid item xs={2.5} className="pt-2.5 selectFixedHeight">
            <SelectTextField
              label={
        ...
        <Grid container spacing={3} className="mt-2 items-end selectFixedHeight">
          <Grid
            className="pt-2.5"
            item
            xs={
          ...
          {values.most_likely_owner_user === AmenitiesEnum.TYPE_MY_OWN ? (
            <Grid item xs={3} className="type-my-own pt-2.5">
              <StyledField
          ...
          {values.topography === AmenitiesEnum.TYPE_MY_OWN ? (
            <Grid item xs={3} className="topography pt-2.5">
              <StyledField
        ...
        <Grid container spacing={3} className="mt-2 items-end">
          ...
          <Grid
            item
            xs={values.frontage === AmenitiesEnum.TYPE_MY_OWN ? 3 : 6}
            className="pt-2.5 selectFixedHeight"
          >
            <SelectTextField
          ...
          {values.frontage === AmenitiesEnum.TYPE_MY_OWN ? (
            <Grid item xs={3} className="fontage pt-2.5">
              <StyledField
        ...
        <Grid container spacing={3} className="mt-2 items-end">
          <Grid item xs={6} className="relative pt-2.5">
            <StyledField
        ...
        <Grid container spacing={3} className="mt-2 items-end">
          <Grid
            item
            xs={values.utilities_select === AmenitiesEnum.TYPE_MY_OWN ? 3 : 6}
            className="utilities pt-2.5 selectFixedHeight"
          >
            <SelectTextField
          ...
          {values.utilities_select === AmenitiesEnum.TYPE_MY_OWN ? (
            <Grid item xs={3} className="pt-2.5">
              <StyledField
          ...
          {showIncludedUtilities && (
            <Grid item xs={6} className="pt-2.5 selectFixedHeight">
              <EvaluationIncludedUtilities updateData={updateData} />
            </Grid>
          )}
        </Grid>
        {showIncludedUtilities && (
          <Grid container spacing={2} className="mt-4 selectFixedHeight">
            <Grid item xs={12}>
              <StyledField
        ...
        <Grid container spacing={3} className="mt-3 items-end">
          ...
          {values.condition === AmenitiesEnum.TYPE_MY_OWN ? (
            <Grid item xs={3} className="other-condition-type pt-2.5">
              <StyledField
          ...
          {values.property_class === AmenitiesEnum.TYPE_MY_OWN ? (
            <Grid item xs={3} className="other-property-class pt-2.5">
              <StyledField
        ...
        {localStorage.getItem('activeType') === 'building_with_land' ? (
          <Grid container spacing={3} className="mt-2 items-end">
            <Grid item xs={6} className="pt-2.5">
              <StyledField
        ...
        {localStorage.getItem('activeType') === 'building_with_land' ? (
          <Grid container spacing={3} className="mt-1 items-end">
            ...
            <Grid item xs={6} className="relative className='pt-2'">
              <StyledField
