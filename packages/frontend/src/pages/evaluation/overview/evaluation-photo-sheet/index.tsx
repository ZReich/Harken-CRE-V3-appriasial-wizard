...
import EvaluationPhotoSheetTable from './evaluation-photo-sheet-table';
import { AppraisalPhotoPagesEnum } from '@/pages/appraisal/overview/OverviewEnum';
import {
  // Photo,
  UploadPhotoPagesResponse,
} from '@/components/interface/appraisal-upload-photo-pages';
...
const EvaluationPhotoSheet = () => {
  ...
  const { mutate } = useMutate<UploadPhotoPagesResponse, any>({
    queryKey: 'upload-photo-pages',
    endPoint: `/evaluations/upload-photo-pages/${id}`,
    requestType: RequestType.POST,
  });
  const { mutateAsync } = useMutate<UpdateProfileUserType, FormDataObject>({
    queryKey: 'upload-files',
    endPoint: `/evaluations/upload-photos`,
    requestType: RequestType.POST,
  });
  const { data } = useGet<any>({
    queryKey: 'evaluations/get-photo-pages',
    endPoint: `evaluations/get-photo-pages/${id}`,
    config: {},
  });
  ...
  const handleSubmit = (values: any) => {
    ...
    if (
      ...
      toast.error('Please complete missing field.');
    }
    ...
    mutate(
      ...
      },
      {
        onSuccess: (res) => {
          setLoader(false);
          navigate(`/evaluation-property-boundaries?id=${id}`);
                    ...
                    <Grid container spacing={2} className='items-end'>
                      ...
                      >
                        <DatePickerComp
                          ...
                          // value={values.photo_date ? moment(values.photo_date, 'MM/DD/YYYY') : null}
                          value={
                        ...
                        {errors && showValidation && (
                          <div className="text-red-500 absolute text-[11px] leading-loose">
...
export default EvaluationPhotoSheet;
