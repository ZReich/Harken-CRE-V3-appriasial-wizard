...
export const EvaluationTaxAssessment = () => {
  ...
  const building = values.zonings?.map((ele: any) => {
    return {
      sq_ft: parseFloat(ele.sq_ft.replace(/,/g, '')),
    };
  });
  ...
  const formattedPriceSquareFootLandOnly = isNaN(priceSqureFootLandOnly)
    ...
    : priceSqureFootLandOnly.toLocaleString('en-US', {
      ...
      useEffect(() => {
    if (localStorage.getItem('activeType') === 'building_with_land' && (values.land_assessment || values.structure_assessment)) {
      setFieldValue('price_square_foot', formattedPriceSquareFoot)
    }
    else if (localStorage.getItem('activeType') === 'land_only' && (values.land_assessment || values.structure_assessment)) {
      setFieldValue('price_square_foot', formattedPriceSquareFootLandOnly)
    }
  }, [values.land_assessment, values.structure_assessment, values.comp_type,values?.land_dimension])
  return (
    <>
      <div>
        <Typography
