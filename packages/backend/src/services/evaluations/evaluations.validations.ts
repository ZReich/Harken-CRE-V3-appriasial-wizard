...
import { improvementSchema } from '../evaluationCostApproachImprovements/evaluationCostImprovement.validations';
import { addScenarioSchema } from '../evaluationScenario/evaluationScenario.valiadations';
...
export const specificationsDetailsSchema = {
	comp_type: Joi.string()
		.valid(CompsEnum.BUILDING_WITH_LAND, CompsEnum.LAND_ONLY)
		.default(CompsEnum.BUILDING_WITH_LAND)
		.required(),
	...
	analysis_type: Joi.string().when(CompsEnum.COMP_TYPE, {
		is: CompsEnum.LAND_ONLY,
		then: Joi.valid(EvaluationsEnum.PRICE_SF, EvaluationsEnum.PRICE_ACRE).required(),
		otherwise: Joi.optional().allow('', null),
	}),
	...
	evaluation_included_utilities: Joi.array().optional(),
...
export const additionalPropertyInfoSchema = {
	building_size: Joi.number().when(CompsEnum.TYPE, {
		is: Joi.valid(CompsEnum.SALE, CompsEnum.LEASE),
		then: Joi.when(CompsEnum.COMP_TYPE, {
			is: CompsEnum.BUILDING_WITH_LAND,
			then: Joi.required(),
			otherwise: Joi.optional().allow(null),
		}),
...
export const evaluationApproachSchema = Joi.object({
	id: Joi.number().optional().allow(null),
	type: Joi.string()
		.valid(
			EvaluationsEnum.INCOME,
			EvaluationsEnum.COST,
			EvaluationsEnum.SALE,
			EvaluationsEnum.LEASE,
			EvaluationsEnum.CAP,
			EvaluationsEnum.MULTI_FAMILY,
			EvaluationsEnum.RENT_ROLL,
		)
		.required(),
...
export const addUpdateApproachSchema = Joi.object({
	approaches: Joi.array().items(evaluationApproachSchema).required(),
});

export const evaluationSetupAddUpdateSchema = Joi.object({
	client_id: Joi.number().required(),
	...
	approaches: Joi.array()
		.items(evaluationApproachSchema)
		.required()
		.custom((value, helper) => {
...
export const uploadImageSchema = Joi.object({
	...
	type: Joi.string().required().valid(UploadEnum.EVALUATION),
...
export const updateExhibitSchema = Joi.object({
	...
	appraisalFilesId: Joi.number().required(),
});
...
export const evalIncomeApproachesSchema = Joi.object().keys({
	id: Joi.number().optional().allow(null),
...
export const saveSalesApproachSchema = Joi.object({
	...
	sales_approach: Joi.object({
		...
		sales_comparison_attributes: Joi.array()
			...
			.optional()
			.allow(null),
		subject_property_adj: Joi.array().items(subAdjustmentsSchema).optional().allow(null),
...
export const evaluationSaveScenarioSchema = Joi.object({
	...
	comp_type: Joi.string()
		...
		.default(CompsEnum.BUILDING_WITH_LAND)
		.required(),
	scenarios: Joi.array().items(addScenarioSchema).min(1).required(),
});
export const SaveReviewSchema = Joi.object({
	evaluation_scenario_id: Joi.number().optional().allow(null),
	...
	review_date: dateFormatValidator.date().optional().allow(null, ''),
});
