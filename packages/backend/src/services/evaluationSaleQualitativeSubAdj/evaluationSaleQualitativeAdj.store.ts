...
import { ISaleSubQualitativeAdj } from './IEvaluationSaleQualitativeAdj';

const SubQualitativeAdj = database.eval_sales_approach_qualitative_sub_adj;
export default class EvaluationSaleSubQualitativeAdjStore {
	...
	public async updateAdjustments(
		...
		try {
			const { adj_key, evaluation_sales_approach_id, ...rest } = attributes;
			const whereClause = { adj_key, evaluation_sales_approach_id };
