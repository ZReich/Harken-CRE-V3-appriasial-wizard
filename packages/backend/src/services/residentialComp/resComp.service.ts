...
import { clusterDetailsSchema, combinedSchema, mapBoundsSchema, resCompsListSchema } from './resComps.validations';
	...
	public getCompsPdfContent = async (compIds: []): Promise<string> => {
		try {
			...
			for (let i = 0; i < compIds?.length; i++) {
				...
				const zoningCode = res_zonings[0]?.zone ? matchCodeOrFallback(zoningsOptions, res_zonings[0]?.zone) : '';
				const subZoneCode = res_zonings[0]?.sub_zone ? matchCodeOrFallback(zoningsOptions, res_zonings[0]?.sub_zone, true) : '';
