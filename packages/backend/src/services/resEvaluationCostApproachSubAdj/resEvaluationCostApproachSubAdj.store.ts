...
import { ICostSubPropertyAdj } from './IResEvaluationCostApproachSubAdj';

const EvaluationCostSubAdj = database.res_eval_cost_approach_subject_adj;
export default class ResEvaluationCostSubAdjStorage {
	...
	public async updateAdjustments(
		...
		try {
			const { adj_key, res_evaluation_cost_approach_id, ...rest } = attributes;
			const whereClause = { adj_key, res_evaluation_cost_approach_id };
